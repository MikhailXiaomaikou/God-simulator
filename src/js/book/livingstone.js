 ...
 *
 * 书信没有情节：一群分散寄居的人，住在海边干旱的山坡上——两顶帐棚、一块旱田、一座石台（锡安的磐石）。
 * 信里的每一句话，在这一日一夜又一日之间成为看得见的光；一座由活石建造的灵宫在他们中间立起来。
 *   雅 1:17   黎明。天上开了一处光：各样美善的恩赐一点一点降下来，落在每个人、田、帐棚、海上。
 *   雅 1:22   听道的人举着一面铜镜看了看，放下，走过穷弟兄身边，随即忘了；彼得提水罐去看顾寡妇孤儿；
 *   雅 2:5    神拣选了世上的贫穷人：行道的妇人背着衣食走到穷弟兄那里——他换上了暖和的衣裳，站起来。
 ...
 *   雅 5:18   农夫在旱田里坐着等候；众人祷告；云聚，雨降，田里出苗，干黄的山坡遍地返青、开花。
 ...
 *   彼前 5:7  子夜。远山后暗红的火光（火炼的试验）；众人背着重担低头；一团灰蓝的冷烟贴着亮处的地面，
 *             从田边向众人游过来（「吼叫的狮子」只在旁白里）；他们站稳抵挡——烟影往回缩、变淡、散了；
 *             把重担卸下——重担化作光升上天去。
 ...
 *             灵宫依旧；又有悔改的人从远处来。
 *   彼后 3:13 早晨。东边天际起了一片新的光——新天新地（神的应许）；遍地开花，众人举手。愿荣耀归给他。阿们！
 *
 * 父不显为人形：只是天上的光与旁白的声音。主乃活石——房角石是一块光的石头，不是人形；
 * 圣山的异象里，子只是一个发光的人形（异象，不是面貌）。圣灵是玩家自己漂游的光。
 * 魔鬼不是活物：只是一团冷暗的烟影（没有眼，没有吼声，只有低低的冷风），被站稳的人抵挡，退去散了。人都无面目。
 *
 * 画面的方位（桌面）：海在左，经文在海上；近地上左边是旱田（0.40–0.53），石台与灵宫在当中（0.57–0.76），
 *   右边是彼得、寡妇孤儿（0.76–0.85），人按纵深分成前后几排；两顶帐棚在最右（≥ 0.93），帐棚前不站人；
 *   中丘左端是一片树林（0.54–0.64）；远山上是圣山的异象（0.83）。
 * 竖屏的手机：经文在上；田在石台之前的平地上（0.43–0.63，v 0.5–0.85），右边的人前后错开（v）；天上的异象放在经文之下。
...
(function (GS) {
  ...
  const PP = {
    farmer: [0.5, 0.62], poor: [0.52, 0.4], hearer: [0.69, 0.5], peter: [0.785, 0.1], doer: [0.83, 0.55], widow: [0.872, 0.25], orphan: [0.9, 0.7],
    hearerAway: [0.44, 0.14], peterW: [0.85, 0.12], doerP: [0.565, 0.42],
    ...
    pAside: [0.63, 0.3], dAside: [0.665, 0.4],
  };
  const XD = {
    ...
    shL0: 0.61, shL1: 0.636, shR0: 0.728, shR1: 0.756, sheepV: 0.58, lion0: 0.462, lion1: 0.585, lionV: 0.6, lionBack: 0.1,
  ...
  const XP = {
    ...
    saint0: 0.66, saint1: 0.78, saintV: 0.66, dL0: 0.36, dL1: 0.42, dR0: 0.955, dR1: 1.0, dLin0: 0.5, dLin1: 0.53, dRin0: 0.86, dRin1: 0.93, dRV: 0.34,
    fire0: 0.44, fire1: 0.54, fireV: 0.42,
    shL0: 0.555, shL1: 0.585, shR0: 0.758, shR1: 0.79, sheepV: 0.36, lion0: 0.47, lion1: 0.58, lionV: 0.44, lionBack: 0.07,
  ...
  function drawLion(ctx) {
    const k = lv('lsLion');
    if (k < 0.01 || !SP) return;
    const [x, y] = lionPt(), s = LS(2);
    const L = (PORT ? 72 : 100) * s, H = (PORT ? 24 : 27) * s, back = lv('lsLionBack');
    // 被抵挡时烟影压扁、拉散
    ...
    gr.addColorStop(0, 'rgb(86,96,126)');
    gr.addColorStop(0.45, 'rgb(46,52,70)');
    gr.addColorStop(1, 'rgb(14,15,22)');
    ctx.fillStyle = gr;
    ctx.globalAlpha = k * 0.72; blob(0); ctx.fill();
    // 当中一团暗而浑的红黑（没有眼睛）
