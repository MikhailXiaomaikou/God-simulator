...
(function (GS) {
  ...
  W.defineLevel('levTents', 'lin', 0.075);  // 「我要在你们中间行走」：荣光走过之处，帐棚一一亮起（26:12）

  // ── 地上的位置（画面宽度的比例）：左 = 东（会幕的门朝东），右 = 西 ──
  const X = {
    leper: 0.387, tentL1: 0.418, hearth: 0.447, tentL2: 0.472, tentMA: 0.512,
  ...
  const FORE = 0.28;          // 故事里的人站得稍靠前（画面更低、更大），不淹没在会众里
  const NEAR_TENTS = [
  ...
  const NEAR_BOOTHS = [0.4, 0.434, 0.49, 0.535, 0.853, 0.9, 0.948, 0.99];
  const MID_BOOTHS = [0.54, 0.595, 0.648, 0.71, 0.77, 0.83, 0.888, 0.945];
  const ROBE = {
    moses: [118, 96, 74], aaron: [134, 112, 88], hp: [54, 72, 142], hpAcc: [228, 192, 104], linen: [234, 230, 218],
  ...
  function aaronLook() {
    ...
    if (S.garb === 2) return { robe: ROBE.linen, accent: [250, 248, 240], hair: 'cloth', glow: 0.55 };
  ...
  function sonLook(id) { return S.sons ? { robe: ROBE.linen, hair: 'cloth', accent: [214, 206, 188] } : { robe: ROBE[id], hair: 'short', accent: null }; }
  ...
  function drawTent(ctx, xf, l, size, seed, lit) {
    ...
    // 夜里门口一盏灯；荣光经过之后透出金光
    const lamp = clamp(nightK() * 1.1, 0, 1) * 0.8 * (0.55 + 0.45 * U.fract(seed * 0.618 + 0.3)) + lit;
    if (lamp > 0.02) {
      SP || sprites();
      ctx.globalCompositeOperation = 'lighter';
      ...
      ctx.fillRect(x - dw, y - dh, dw * 2, dh);
      const g = hw * (1.8 + lit * 1.6);
      ctx.globalAlpha = Math.min(1, lamp) * fl * (0.45 + 0.25 * lit);
  ...
  function drawCourt(ctx) {
    ...
    const linen = css([236, 232, 220], 2), fold = css([168, 160, 146], 2, 0.3), post = css([120, 96, 66], 2), cap = css([214, 214, 220], 2, 1, 0.1);
    // 后面一道
    const N = 26;
    ctx.fillStyle = linen;
    ctx.beginPath();
  ...
  // 云柱画在两张离屏的画布上（云：常规叠加；火与荣光：加亮叠加），每十分之一秒重画一次——云走得很慢
  const PC = { a: null, b: null, t: -99, w: 0, h: 0, glow: false };
  function pillarGeom() {
    const G = tabGeom(), L = G.x1 - G.x0, cx = PILLAR_X * W.w, base = pillarBase();
    const pt = port(), top = pt ? W.h * 0.16 : -W.h * 0.12;
  ...
  function drawPillar(ctx) {
    SP || sprites();
    const P = pillarGeom();
    const bx = P.cx - P.w0 * 2.5, bw = P.w0 * 5, by = Math.max(-20, P.top - P.w0), bh = P.base + P.w0 * 2.5 - by;
    const sc = W.w < 600 ? 0.6 : 0.5;
    const cw = Math.max(8, Math.ceil(bw * sc)), ch = Math.max(8, Math.ceil(bh * sc));
    if (!PC.a || PC.a.width !== cw || PC.a.height !== ch) { PC.a = cnv(cw, ch); PC.b = cnv(cw, ch); PC.t = -99; }
    if (Math.abs(W.t - PC.t) >= 0.1 || PC.w !== W.w || PC.h !== W.h) {
      PC.t = W.t; PC.w = W.w; PC.h = W.h;
      const ga = PC.a.getContext('2d'), gb = PC.b.getContext('2d');
      for (const g of [ga, gb]) { g.setTransform(1, 0, 0, 1, 0, 0); g.clearRect(0, 0, cw, ch); g.setTransform(sc, 0, 0, sc, -bx * sc, -by * sc); }
      PC.glow = renderPillar(ga, gb, P);
    }
  ...
  function renderPillar(ctx, gl, P) {
    ...
    const lx = litX() >= cx ? 1 : -1;
    const fadeTop = ph => 1 - smoothstep(0.5, 1, ph);
    const aLit = 0.42 + 0.32 * day, aSh = 0.44 + 0.3 * day;
    const N = 30;
    const puffAt = (i, ph) => {
      const R = PUFF[i];
      const y = base - ph * Hh;
      const sw = Math.sin(W.t * 0.15 + ph * 3) * w0 * 0.07 + (R[0] - 0.5) * w0 * 0.3;
      const r = w0 * (0.55 + 0.55 * ph) * (0.85 + 0.3 * R[1]) * (1 + 0.04 * Math.sin(W.t * 0.6 + i));
      return [cx + sw, y, r];
    };
    // 背光的一面
    for (let i = 0; i < N; i++) {
      const ph = U.fract(i / N + W.t * 0.006 + PUFF[i][2] * 0.02);
      const [x, y, r] = puffAt(i, ph);
      const a = Math.min(1, ph * 10) * fadeTop(ph);
      spr(ctx, cs.shade, x - lx * r * 0.22, y + r * 0.14, r, a * aSh);
    }
    // 歇在帐幕顶上的云
    const REST = [[0.1, 0.17, 0.1], [0.26, 0.24, -0.05], [0.44, 0.3, -0.16], [0.6, 0.27, -0.08], [0.76, 0.22, 0.02], [0.9, 0.16, 0.12], [0.52, 0.34, -0.34]];
    for (const q of REST) {
      const x = G.x0 + L * q[0], r = L * q[1] * (1 + 0.03 * Math.sin(W.t * 0.5 + q[0] * 9)), y = G.top + L * q[2];
      spr(ctx, cs.shade, x - lx * r * 0.2, y + r * 0.12, r, aSh * 0.85);
    }
    for (const q of REST) {
      const x = G.x0 + L * q[0], r = L * q[1] * (1 + 0.03 * Math.sin(W.t * 0.5 + q[0] * 9)), y = G.top + L * q[2];
      spr(ctx, cs.lit, x + lx * r * 0.12, y - r * 0.14, r * 0.8, aLit * 0.95);
    }
    // 迎光的一面
    for (let i = 0; i < N; i++) {
      const ph = U.fract(i / N + W.t * 0.006 + PUFF[i][2] * 0.02);
      const [x, y, r] = puffAt(i, ph);
      const a = Math.min(1, ph * 10) * fadeTop(ph);
      spr(ctx, cs.lit, x + lx * r * 0.14, y - r * 0.1, r * 0.84, a * aLit);
    }
    // 香的烟云（16:13）：金色的香烟自帐幕中升起，遮掩施恩座
    if (inc > 0.01) {
      for (let j = 0; j < 3; j++) smoke(ctx, G.x0 + L * (0.3 + 0.2 * j), G.top + 4 * s, inc, Hh * 0.35, L * 0.12, 7 + j * 3, SP.incense, 0.11, 0.55);
      spr(gl, SP.gold, cx, G.top, L * 0.9, inc * 0.35);
    }
    // 云中的火（夜）与荣光（金）：画在加亮的一层
    if (nk > 0.02 || g > 0.02) {
      for (let i = 0; i < 24; i++) {
        const ph = U.fract(i / 24 + W.t * 0.01);
        const y = base - ph * Hh * 0.9, sw = Math.sin(W.t * 0.5 + i * 1.3) * w0 * 0.1;
        const fl = 0.8 + 0.2 * Math.sin(W.t * 3.1 + i * 2.7);
        const a = Math.min(1, ph * 8) * fadeTop(ph) * (1 - ph * 0.45);
        spr(gl, SP.fireSoft, cx + sw, y, w0 * (0.75 + 0.45 * ph), a * nk * 0.32 * fl);
        spr(gl, SP.gold, cx + sw, y, w0 * (0.75 + 0.5 * ph), a * g * (0.12 + 0.24 * nk) * fl);
      }
      spr(gl, SP.warm, cx, base - Hh * 0.18, w0 * 1.6, nk * 0.35);
      spr(gl, SP.gold, cx, base - 6 * s, w0 * 2.4, g * (0.22 + 0.25 * nk));
    }
    ctx.globalAlpha = 1; gl.globalAlpha = 1;
    return nk > 0.02 || g > 0.02 || inc > 0.01;
  }
  ...
  function drawLights(ctx) {
    ...
    if (nk > 0.02) spr(ctx, SP.fire, PILLAR_X * W.w, pillarBase() + 10 * s, mm * 0.3, nk * 0.2);
    ...
    if (hl > 0.01) spr(ctx, SP.gold, PILLAR_X * W.w, gY(2, PILLAR_X) - 10 * s, mm * 0.32, hl * 0.16);
  ...
  function walkerX(v) { return v < 0.28 ? lerp(PILLAR_X, 0.985, v / 0.28) : lerp(0.985, 0.39, (v - 0.28) / 0.72); }
  function tentLit(xf) {
    const v = W.lv.levTents;
    if (v <= 0) return 0;
    const need = xf >= PILLAR_X ? 0.28 * (xf - PILLAR_X) / (0.985 - PILLAR_X) : 0.28 + 0.72 * (0.985 - xf) / (0.985 - 0.39);
  ...
  function drawWalker(ctx) {
    ...
    const xf = walkerX(v), s = LS(2), x = xf * W.w, y = gY(2, xf) + 2 * s, H = 110 * s, w = 34 * s;
    ctx.globalCompositeOperation = 'lighter';
    ctx.globalAlpha = env * 0.55;
    ctx.drawImage(SP.column, x - w / 2, y - H, w, H + 6 * s);
    spr(ctx, SP.gold, x, y - H * 0.35, w * 2.2, env * 0.45);
    spr(ctx, SP.white, x, y - H * 0.4, w * 0.8, env * 0.5);
    // 脚下的光
    ctx.globalAlpha = env * 0.5;
    ctx.drawImage(SP.gold, x - w * 2, y - w * 0.3, w * 4, w * 0.9);
    ctx.globalCompositeOperation = 'source-over';
    ctx.globalAlpha = 1;
  }
  ...
  function drawTransients(ctx) {
    ...
    SP || sprites();
    const mm = M(), u = Math.max(0.55, W.unit), s2 = LS(2);
    for (const e of FXL) {
      const q = clamp(e.t / e.dur, 0, 1);
      ctx.globalCompositeOperation = 'lighter';
      if (e.type === 'voice') {
        // 从会幕中呼叫：门口一团光，几道光环自门口荡开
        ...
        for (let k = 0; k < 3; k++) {
          ...
          if (p <= 0 || p >= 1) continue;
          ctx.globalAlpha = (1 - p) * 0.6; ctx.lineWidth = Math.max(1, 2.2 * u * (1 - p));
          ctx.beginPath(); ctx.ellipse(x, y, p * mm * 0.5, p * mm * 0.16, 0, 0, TAU); ctx.stroke();
        }
      } else if (e.type === 'firefall') {
        // 有火从耶和华面前出来（9:24）：自云中直落坛上
        const A = altarGeom(), x0 = PILLAR_X * W.w, y0 = pillarBase() - Math.min(W.h * 0.2, 150 * s2), x1 = A.x, y1 = A.top;
        const cxp = lerp(x0, x1, 0.72), cyp = y0 - 12 * s2;       // 自云中出来，弯下落在坛上
        const head = smoothstep(0, 0.26, q), env = 1 - smoothstep(0.55, 1, q);
        const N = 26;
        for (let i = 0; i < N; i++) {
          const k = i / (N - 1) * head, m = 1 - k;
          const cx = m * m * x0 + 2 * m * k * cxp + k * k * x1, cy = m * m * y0 + 2 * m * k * cyp + k * k * y1;
          const r = (9 + 14 * k) * s2 * (0.8 + 0.2 * Math.sin(e.t * 30 + i));
          spr(ctx, SP.fire, cx, cy, r * 2.2, env * 0.8);
          spr(ctx, SP.white, cx, cy, r * 0.9, env * 0.75);
        }
        if (q > 0.25) {
          const bq = clamp((q - 0.25) / 0.75, 0, 1);
          spr(ctx, SP.gold, x1, y1, (80 + 260 * bq) * s2, (1 - bq) * 0.9);
          spr(ctx, SP.white, x1, y1, (30 + 80 * bq) * s2, (1 - bq) * 0.9);
        }
      } else if (e.type === 'strike') {
        // 拿答、亚比户：火自门口出来——只是一道白金的光
        const G = tabGeom(), x0 = G.x0, y0 = G.g0 - G.H * 0.5;
        const env = smoothstep(0, 0.12, q) * (1 - smoothstep(0.35, 1, q));
        for (const id of e.ids) {
          const h = headOf(id, 0.6);
          if (!h) continue;
          for (let i = 0; i < 8; i++) { const k = i / 7; spr(ctx, SP.white, lerp(x0, h[0], k), lerp(y0, h[1], k), (14 - 6 * k) * s2, env * 0.8); }
          spr(ctx, SP.gold, h[0], h[1], 50 * s2, env * 0.9);
        }
      } else if (e.type === 'oil') {
        // 把膏油倒在亚伦的头上（8:12）
        ...
        for (let i = 0; i < 2; i++) {
          ...
          const bx = lerp(h[0] + dir * (6 + i * 4) * s2, A.x, flyK), by = lerp(h[1] - i * 3 * s2, A.top - 20 * s2, flyK) - Math.sin(flyK * Math.PI) * 30 * s2;
          const flap = Math.sin(e.t * (flyK > 0 ? 18 : 3) + i) * 2.2 * s2;
          ctx.globalAlpha = 1 - smoothstep(0.9, 1, q);
          ctx.fillStyle = css([236, 232, 226], 2, 1, 0.2);
          ctx.beginPath(); ctx.ellipse(bx, by, 2.4 * s2, 1.6 * s2, 0, 0, TAU); ctx.fill();
          ctx.beginPath(); ctx.moveTo(bx - 3.2 * s2, by - flap); ctx.lineTo(bx, by - 0.6 * s2); ctx.lineTo(bx + 3.2 * s2, by - flap); ctx.lineTo(bx, by + 0.4 * s2); ctx.closePath(); ctx.fill();
        }
        ...
        for (let k = 0; k < 7; k++) {
          ...
          for (let j = 0; j < 5; j++) {
            ...
            ctx.fillRect(xx - 0.9 * u, yy - 0.9 * u, 1.8 * u, 1.8 * u);
          }
          if (p > 0.8) spr(ctx, SP.white, b[0], b[1], 12 * s2, (1 - p) * 3);
        }
      } else if (e.type === 'bird') {
        // 又把活鸟放在田野里（14:7）：一只白鸟自祭司手中飞起，飞过海面，没入天空
        const p = U.easeInOut(q);
        const x0 = e.x0, y0 = e.y0, x2 = W.w * 0.08, y2 = W.h * 0.14, x1 = lerp(x0, x2, 0.3), y1 = y0 - W.h * 0.28;
        const bx = (1 - p) * (1 - p) * x0 + 2 * (1 - p) * p * x1 + p * p * x2, by = (1 - p) * (1 - p) * y0 + 2 * (1 - p) * p * y1 + p * p * y2;
        const sz = lerp(4.2, 1.6, p) * Math.max(0.8, u) * (W.w < 600 ? 1.2 : 1), flap = Math.sin(e.t * 16) * sz;
        ctx.globalCompositeOperation = 'source-over';
        ctx.globalAlpha = 1 - smoothstep(0.85, 1, q);
        ctx.strokeStyle = 'rgb(248,246,240)'; ctx.lineWidth = Math.max(1, sz * 0.45); ctx.lineCap = 'round';
        ...
        ctx.globalCompositeOperation = 'lighter';
        spr(ctx, SP.white, bx, by, sz * 5, (1 - q) * 0.35);
      } else if (e.type === 'sins') {
        ...
        for (let i = 0; i < e.src.length; i++) {
          ...
          for (let j = 0; j < 3; j++) {
            ...
            ctx.globalAlpha = env * Math.sin(p * Math.PI) * 0.75;
            ctx.beginPath(); ctx.arc(xx, yy, 1.5 * u, 0, TAU); ctx.fill();
          }
        }
        ctx.globalCompositeOperation = 'lighter';
        spr(ctx, SP.violet, g[0], g[1], 20 * s2, env * smoothstep(0.3, 0.7, q) * 0.5);
        ...
        const x0 = X.gate * W.w, x1 = X.courtR * W.w, h = 30 * s2, lift = 6 * s2;
        ...
        // 禧年的角（25:9）：吹角的人口边一支弯弯的羊角
        const env = smoothstep(0, 0.06, q) * (1 - smoothstep(0.88, 1, q));
        for (const id of e.ids) {
          const f = fig(id), h = headOf(id, 0.9);
          if (!f || !h) continue;
          const dir = f.facing || 1, L = 12 * s2;
          ctx.globalCompositeOperation = 'source-over';
          ctx.globalAlpha = env;
          ctx.strokeStyle = css([226, 206, 170], 2, 1, 0.15); ctx.lineCap = 'round';
          ctx.lineWidth = Math.max(1, 2.2 * s2);
          ctx.beginPath(); ctx.moveTo(h[0] + dir * 2 * s2, h[1] + 2 * s2); ctx.quadraticCurveTo(h[0] + dir * L * 0.9, h[1] + 1 * s2, h[0] + dir * L, h[1] - L * 0.7); ctx.stroke();
          ctx.globalCompositeOperation = 'lighter';
          const blast = Math.max(0, Math.sin(e.t * 1.35 * Math.PI * 0.5 - 0.3));
          spr(ctx, SP.gold, h[0] + dir * L, h[1] - L * 0.7, 18 * s2, env * blast * 0.8);
        }
      } else if (e.type === 'waves') {
        // 角声：扁扁的光环贴着大地向四方荡开（25:9 要在遍地发出角声）
        ctx.strokeStyle = 'rgb(255,226,168)';
        for (const pt of e.pts) {
          for (let k = 0; k < 2; k++) {
            const p = clamp(q * 1.25 - k * 0.2, 0, 1);
            if (p <= 0 || p >= 1) continue;
            ctx.globalAlpha = (1 - p) * (1 - p) * 0.7;
            ctx.lineWidth = Math.max(1, (2.6 - k) * u * (1 - p * 0.6));
            ctx.beginPath(); ctx.ellipse(pt[0], pt[1], p * e.R, p * e.R * 0.26, 0, 0, TAU); ctx.stroke();
          }
          spr(ctx, SP.gold, pt[0], pt[1], 30 * s2, (1 - q) * 0.8);
        }
      } else if (e.type === 'sweep') {
        // 一道光沿着地面走过（圣洁、禧年、地是我的）
        const env = 1 - smoothstep(0.7, 1, q), hx = lerp(e.x0, e.x1, U.easeOut(q)), l = e.l == null ? 2 : e.l;
        ctx.strokeStyle = 'rgb(255,232,176)';
        ctx.lineWidth = Math.max(1, 2 * W.unit);
        ctx.globalAlpha = env * 0.5;
        ctx.beginPath();
        for (let i = 0; i <= 18; i++) { const xf = lerp(e.x0, hx, i / 18); const yy = gY(l, xf) - 2; if (i) ctx.lineTo(xf * W.w, yy); else ctx.moveTo(xf * W.w, yy); }
        ctx.stroke();
        spr(ctx, SP.gold, hx * W.w, gY(l, hx), (90 * LS(l) + 20) / 2, env * 0.8);
      } else if (e.type === 'lift') {
        // 他必蒙赦免：一点光自人身上升起
  ...
  // 一道扁扁的光环贴着地面荡开（代替正圆的光环）
  function halo(b, xf, y, R, dur) { fl(b, { type: 'waves', pts: [[xf * W.w, y]], R, dur: dur || 3 }); }
  ...
  function setup() {
    ...
    const lv = { deep: 1, light: 1, gather: 1, dayNight: 1, vault: 1, clouds: 0.35, land: 1, grass: 1, herbs: 0.35, trees: 0, lights: 1, moon: 1, stars: 1, life: 1, good: 0, given: 1, sabbath: 0,
      ...
      levGlory: 0.28, levFire: 0.75, levHoly: 0, levIncense: 0, levDays: 0, levBooths: 0, levLamp: 0, levHearth: 0, levTents: 0 };
    ...
    const c = C();
    c.clear({ fade: false });
    // 摩西站在院子的门前；亚伦和他儿子在东边自己的帐棚前（民 3:38）；一位作祭司的在坛旁
    putMoses(0.566, { facing: 1, from: 'none' });
    putAaron(0.525, { facing: 1, from: 'none' });
    add('nadab', Object.assign({ label: '拿答', sex: 'm', age: 'adult', x: 0.497, facing: 1, glow: 0.2, from: 'none' }, sonLook('nadab')));
    add('abihu', Object.assign({ label: '亚比户', sex: 'm', age: 'adult', x: 0.484, facing: 1, glow: 0.2, from: 'none' }, sonLook('abihu')));
    add('eleazar', Object.assign({ label: '以利亚撒', sex: 'm', age: 'adult', x: 0.638, facing: -1, glow: 0.25, from: 'none' }, sonLook('eleazar')));
    add('ithamar', Object.assign({ label: '以他玛', sex: 'm', age: 'adult', x: 0.536, facing: 1, glow: 0.2, from: 'none' }, sonLook('ithamar')));
    add('offerer', { label: '献祭的人', sex: 'm', age: 'adult', x: 0.43, facing: 1, robe: ROBE.offerer, glow: 0.25, v: FORE, from: 'none' });
    add('woman', { label: '他的妻子', sex: 'f', age: 'adult', x: 0.407, facing: 1, robe: ROBE.woman, glow: 0.2, v: FORE, from: 'none' });
    c.crowd('campL', { n: 11, x0: 0.392, x1: 0.545, layer: 2, label: '以色列人', from: 'none' });
    ...
    herd('flock', { kind: 'sheep', n: 6, x0: 0.9, x1: 1.0, layer: 2, label: '羊群', from: 'none', v: 0.35 });
    avoid([0.37, 0.87]);
  }
  ...
  const STAGES = [
    ...
    {
      kind: 'call', utter: '耶和华从会幕中呼叫摩西', cmd: 'listen --from 会幕 --to 摩西  # 你晓谕以色列人说', ref: '1:1',
      verse: [
      ...
      apply(c) {
        T(c, [
          ...
          [5, b => {
            animal('bull', 'cow', 0.405, { label: '公牛', facing: 1, v: FORE, from: b.instant ? 'none' : 'fade' });
            walk('offerer', X.gate + 0.004, { speed: 0.028 });
            U.safe('cast.follow', () => C().follow('bull', 'offerer', 0.032));
            sfx(b, 'cow');
          }],
          [8, () => pose('moses', 'stand')],
          ...
          [14.2, b => {
            ...
            walk('offerer', 0.43, { speed: 0.03 });
            pose('eleazar', 'stand');
            hold('woman', 'jar');
            walk('woman', X.gate + 0.01, { speed: 0.032 });
          }],
          ...
          [22.2, () => { pose('eleazar', 'stand'); hold('woman', null); walk('woman', 0.412, { speed: 0.03, pose: 'sit' }); pose('offerer', 'sit'); W.set('levFire', 0.9); }],
        ]);
      },
    },
    ...
    // ── 4—7 坛上的火常常烧着：赎罪祭、认罪、一整夜的火 ───────
    {
      ...
      apply(c) {
        T(c, [
          [0, b => { W.set('levFire', 1.25, b.instant); if (!b.instant) fx().sparkle(altarGeom().x, altarGeom().top - 14, 30, [255, 210, 150], 14, 'air'); W.goTo(0.76, 12, b.instant); sfx(b, 'fire'); }],
          [2.4, b => {
            pose('offerer', 'stand');
            animal('shegoat', 'goat', 0.41, { label: '母山羊', facing: 1, v: FORE, from: b.instant ? 'none' : 'fade' });
            walk('offerer', X.gate + 0.004, { speed: 0.028 });
          ...
          [14.6, b => { W.goTo(0.0, 6, b.instant); }],
          [15.8, b => {
            say(b, [{ text: '他有了罪的时候，就要承认所犯的罪……<br>至于他的罪，祭司要为他赎了。', ref: '利未记 5:5–6', hold: 6 }]);
            fl(b, { type: 'lift', id: 'offerer', dur: 4 });
          }],
          [17.6, () => { pose('offerer', 'stand'); walk('offerer', 0.43, { speed: 0.03, pose: 'sit' }); pose('eleazar', 'stand'); }],
          [20.6, b => { W.goTo(0.28, 9, b.instant); pose('eleazar', 'carry'); if (!b.instant) fx().sparkle(altarGeom().x, altarGeom().top - 6, 24, [255, 190, 110], 10, 'air'); sfx(b, 'fire', { soft: true }); }],
      ...
      apply(c) {
        T(c, [
          [0, b => {
            ...
            crowdWalk('campR', 0.43, 0.545, { speed: 0.036 });
            crowdWalk('campL', 0.39, 0.53, { speed: 0.03 });
            sfx(b, 'crowd', { soft: true });
          }],
          [1, () => {
            walk('moses', X.laver - 0.018, { speed: 0.03 });
            walk('aaron', X.laver + 0.012, { speed: 0.03 });
            walk('nadab', X.laver + 0.03, { speed: 0.03 });
            walk('abihu', X.laver + 0.022, { speed: 0.032 });
            walk('eleazar', X.laver - 0.008, { speed: 0.03 });
            walk('ithamar', X.laver + 0.004, { speed: 0.034 });
          }],
          [8.3, b => {
          ...
          [18.2, () => {
            ...
            walk('eleazar', X.tabL - 0.024, { speed: 0.025, pose: 'sit' });
            walk('ithamar', X.tabL - 0.036, { speed: 0.025, pose: 'sit' });
            walk('nadab', X.tabL - 0.048, { speed: 0.025, pose: 'sit' });
            walk('abihu', X.tabL - 0.06, { speed: 0.025, pose: 'sit' });
            walk('moses', X.altar - 0.03, { speed: 0.025 });
          }],
          [21, b => {
            say(b, [{ text: '七天你们要昼夜住在会幕门口，遵守耶和华的吩咐……', ref: '利未记 8:35', hold: 6 }]);
            W.set('levDays', 1, b.instant);
            W.goTo(0.26, 10, b.instant);
          }],
        ]);
      },
    },
    ...
    // ── 9 第八天：有火从耶和华面前出来（本卷的签名）────────────
    {
      ...
      verse: [
        ...
        { text: '有火从耶和华面前出来，在坛上烧尽燔祭和脂油；<br>众民一见，就都欢呼，俯伏在地。', ref: '利未记 9:24', hold: 9 },
      ],
      apply(c) {
        T(c, [
          [0, b => {
            ...
            pose('aaron', 'stand');
            walk('aaron', X.altar - 0.018, { speed: 0.045, pose: 'raise' });
            walk('moses', X.altar - 0.034, { speed: 0.03 });
            people('stand', 1);
          }],
          [2.4, () => face('aaron', -1)],
          [3.6, () => { walk('aaron', X.tabL + 0.004, { speed: 0.045 }); walk('moses', X.tabL - 0.004, { speed: 0.045 }); }],
          [5.8, () => { rm('aaron'); rm('moses'); }],
          [7, b => {
            putAaron(X.tabL + 0.004, { from: b.instant ? 'none' : 'fade' });
            putMoses(X.tabL - 0.004, { from: b.instant ? 'none' : 'fade' });
            walk('aaron', X.tabL - 0.02, { speed: 0.02, pose: 'raise' });
            walk('moses', X.tabL - 0.036, { speed: 0.02, pose: 'raise' });
            W.set('levGlory', 1, b.instant);
            halo(b, PILLAR_X, gY(2, PILLAR_X), M() * 0.9, 3.2);
            sfx(b, 'angel');
          }],
          [8.9, b => {
            fl(b, { type: 'firefall', dur: 3.4 });
            W.set('levFire', 2.1, b.instant);
            if (!b.instant) { W.flash = Math.max(W.flash || 0, 0.85); W.shake = Math.max(W.shake || 0, 0.5); }
            sfx(b, 'fire', { size: 2 }); sfx(b, 'thunder', { soft: true, far: true });
          }],
          [10.2, b => { people('raise', 1); sfx(b, 'crowd'); }],
          [12.4, () => {
            people('fall', 1);
            ['moses', 'aaron', 'nadab', 'abihu', 'eleazar', 'ithamar'].forEach(id => { face(id, 1); pose(id, 'fall'); });
          }],
          [15, () => { W.set('levFire', 1.1); W.set('levGlory', 0.5); }],
        ]);
      },
    },
      ...
      apply(c) {
        T(c, [
          [0, b => {
            ...
            walk('nadab', X.altar + 0.012, { speed: 0.03 }); walk('abihu', X.altar + 0.026, { speed: 0.03 });
          }],
          [3.2, () => { hold('nadab', 'torch'); hold('abihu', 'torch'); }],
          [4, () => { walk('nadab', X.tabL - 0.004, { speed: 0.03, pose: 'carry' }); walk('abihu', X.tabL - 0.017, { speed: 0.03, pose: 'carry' }); }],
          [8.3, b => {
            fl(b, { type: 'strike', ids: ['nadab', 'abihu'], dur: 2.4 });
            if (!b.instant) W.flash = Math.max(W.flash || 0, 0.5);
            hold('nadab', null); hold('abihu', null);
          ...
          [11, () => { face('aaron', 1); pose('aaron', 'bow'); walk('moses', X.altar - 0.004, { speed: 0.025 }); }],
      ...
      apply(c) {
        T(c, [
          [0, b => {
            ...
            fl(b, { type: 'sweep', x0: PILLAR_X, x1: 0.36, dur: 3.5 }); fl(b, { type: 'sweep', x0: PILLAR_X, x1: 1.0, dur: 3 });
            fl(b, { type: 'sweep', x0: PILLAR_X, x1: 0.5, l: 1, dur: 3.2 });
            halo(b, PILLAR_X, gY(2, PILLAR_X), Math.hypot(W.w, W.h) * 0.8, 3.6);
            people('gaze');
            sfx(b, 'chime');
          }],
          [4, () => { people('stand'); }],
          ...
          [13.4, b => {
            add('mother', { label: '生育的妇人', sex: 'f', age: 'adult', x: X.tentL1 + 0.004, facing: 1, robe: ROBE.mother, glow: 0.3, carry: 'baby', v: FORE, from: b.instant ? 'none' : 'fade' });
            walk('mother', X.gate + 0.006, { speed: 0.03 });
            fl(b, { type: 'doves', dur: 8.5 });
          }],
          [15, b => say(b, [{ text: '她的力量若不够献一只羊羔，她就要取两只斑鸠或是两只雏鸽……<br>祭司要为她赎罪，她就洁净了。', ref: '利未记 12:8', hold: 6.5 }])],
          [19.6, b => { face('mother', 1); pose('mother', 'bow'); walk('ithamar', X.gate + 0.026, { speed: 0.03, pose: 'raise' }); face('ithamar', -1); sfx(b, 'dove'); }],
          [21.6, () => { pose('mother', 'stand'); glow('mother', 0.45); walk('mother', X.tentL1 + 0.012, { speed: 0.03 }); }],
          [22.4, b => {
            say(b, [{ text: '灾病在他身上的日子，他便是不洁净；<br>他既是不洁净，就要独居营外。', ref: '利未记 13:46', hold: 5.5 }]);
            add('leper', { label: '长大麻风的人', sex: 'm', age: 'adult', x: X.tentL2 + 0.004, facing: -1, robe: ROBE.leper, glow: 0, v: FORE, from: b.instant ? 'none' : 'fade' });
      ...
      apply(c) {
        T(c, [
          [0, b => {
            ...
            W.set('levHoly', 0.35);
            walk('eleazar', X.leper + 0.024, { speed: 0.042 });
          ...
          [10.4, b => {
            ...
            if (h) fl(b, { type: 'bird', x0: h[0], y0: h[1], dur: 9 });
          ...
          [13, b => {
            ...
            pose('eleazar', 'stand');
            walk('leper', X.tentL2 + 0.018, { speed: 0.026, pose: 'sit' });
    ...
    // ── 16—17 赎罪日：香的烟云；这羊要担当他们一切的罪孽 ─────────
    {
      ...
      verse: [
        ...
        { text: '要把这羊放在旷野，这羊要担当他们一切的罪孽，带到无人之地。', ref: '利未记 16:22', hold: 6.5 },
      ],
      apply(c) {
        T(c, [
          [0, b => {
            ...
            walk('aaron', X.tabL + 0.004, { speed: 0.03 });
            animal('goatA', 'goat', X.altar - 0.03, { label: '归与阿撒泻勒的羊', facing: 1, v: FORE, from: b.instant ? 'none' : 'fade', col: [66, 56, 50] });
            add('sent', { label: '所派的人', sex: 'm', age: 'adult', x: X.altar - 0.052, facing: 1, robe: ROBE.sent, glow: 0.2, v: FORE, from: b.instant ? 'none' : 'fade' });
            people('kneel', 1);
            sfx(b, 'goat');
          }],
          [2.2, b => { rm('aaron'); W.set('levIncense', 1, b.instant); }],
          [4.4, b => {
            putAaron(X.tabL + 0.004, { from: b.instant ? 'none' : 'fade' });
            walk('aaron', X.altar - 0.012, { speed: 0.03 });
            W.set('levIncense', 0);
          }],
          [7.6, b => {
          ...
          [11, b => {
            pose('aaron', 'stand');
            walk('sent', 1.07, { speed: 0.03 });
            U.safe('cast.follow', () => C().follow('goatA', 'sent', 0.028));
            sfx(b, 'wind', { soft: true });
          }],
          [15.8, b => {
            say(b, [{ text: '因在这日要为你们赎罪，使你们洁净。<br>你们要在耶和华面前得以洁净，脱尽一切的罪愆。', ref: '利未记 16:30', hold: 6.5 }]);
            people('raise', 1);
            fl(b, { type: 'sweep', x0: 0.36, x1: 1.0, dur: 4 });
            W.set('levHoly', 0.5);
            sfx(b, 'chime');
          }],
          [19, () => { people('stand'); dressAaron(1); }],
          [22.6, b => say(b, [{ text: '因为活物的生命是在血中。我把这血赐给你们，<br>可以在坛上为你们的生命赎罪……', ref: '利未记 17:11', hold: 6 }])],
          [25.5, () => { rm('goatA'); rm('sent'); crowdMill('campL', true); crowdMill('campR', true); }],
        ]);
      },
    },
    ...
    // ── 18—20 要爱人如己：寄居的外人坐到营火旁 ───────────────────
    {
      ...
      apply(c) {
        T(c, [
          ...
          [4.5, () => { walk('offerer', X.tentL2 + 0.004, { speed: 0.03 }); face('leper', -1); }],
          ...
          [13.8, b => {
            ...
            walk('stranger', X.hearth - 0.022, { speed: 0.024 });
            walk('woman', X.hearth - 0.004, { speed: 0.02 });
          }],
          [18.4, () => { face('woman', -1); face('stranger', 1); pose('woman', 'point'); }],
          [20.2, () => {
            pose('woman', 'sit'); pose('stranger', 'sit');
            pose('offerer', 'stand');
            walk('offerer', X.hearth + 0.014, { speed: 0.02, pose: 'sit' }); walk('leper', X.hearth + 0.026, { speed: 0.02, pose: 'sit' });
          }],
          [22.8, b => {
      ...
      apply(c) {
        T(c, [
          [0, b => {
            ...
            walk('aaron', X.tabL - 0.014, { speed: 0.03 }); walk('eleazar', X.tabL - 0.03, { speed: 0.03 }); walk('ithamar', X.tabL - 0.044, { speed: 0.03 });
          ...
          [1.6, b => {
            if (!b.instant) {
              const sz = Math.min(W.w * 0.2, W.h * 0.2), cx = PILLAR_X, cy = Math.max(W.h * 0.2, pillarBase() - W.h * 0.3);
          ...
          [3.6, () => { people('bow', 1); ['aaron', 'eleazar', 'ithamar', 'moses'].forEach(id => { face(id, 1); pose(id, 'bow'); }); }],
          [12, () => { people('stand'); ['aaron', 'eleazar', 'ithamar', 'moses'].forEach(id => pose(id, 'stand')); W.set('levGlory', 0.45); }],
        ]);
      },
    },

    // ── 23—24 住棚节：棕树的枝子、河旁的柳枝；灯常常点着 ───────────
    {
      kind: 'cmd', utter: '你们要住在棚里七日', cmd: 'mkdir -p 棚/{1..7}  # 在耶和华面前欢乐七日', ref: '23:42',
      verse: [
        { text: '「你晓谕以色列人说：耶和华的节期，你们要宣告为圣会的节期。', ref: '利未记 23:2', hold: 5 },
      ...
      apply(c) {
        T(c, [
          ...
          [6.2, b => { W.set('levBooths', 1, b.instant); sfx(b, 'build'); sfx(b, 'wind', { soft: true }); }],
    ...
    // ── 25 禧年：大发角声，在遍地宣告自由 ─────────────────────────
    {
      kind: 'bless', utter: '在遍地给一切的居民宣告自由', cmd: 'broadcast 自由 --to 遍地 --year 50  # 各归本家', ref: '25:10',
      verse: [
      ...
      apply(c) {
        T(c, [
          [0, b => {
            ...
            pose('aaron', 'stand');
            S.horns = 1;
            add('levi1', { label: '吹角的利未人', sex: 'm', age: 'adult', x: 0.396, facing: -1, robe: ROBE.levite, hair: 'cloth', glow: 0.2, from: b.instant ? 'none' : 'fade' });
            add('levi2', { label: '吹角的利未人', sex: 'm', age: 'adult', x: 0.955, facing: 1, robe: ROBE.levite, hair: 'cloth', glow: 0.2, from: b.instant ? 'none' : 'fade' });
          ...
          [9.8, b => {
            people('raise');
            fl(b, { type: 'sweep', x0: 0.36, x1: 1, dur: 5 }); fl(b, { type: 'sweep', x0: 0.5, x1: 1, l: 1, dur: 5 }); fl(b, { type: 'sweep', x0: 0.12, x1: 1, l: 0, dur: 6 });
            W.set('levHoly', 0.7);
          ...
          [13, () => {
            people('stand');
            crowdMill('campL', true); crowdMill('campR', true);
            crowdWalk('campL', 0.39, 0.54, { speed: 0.03 }); crowdWalk('campR', 0.86, 0.99, { speed: 0.03 });
            ['eleazar', 'ithamar', 'levi1', 'levi2'].forEach(id => pose(id, 'stand'));
            walk('offerer', X.tentL1 + 0.012, { speed: 0.025 }); walk('woman', X.tentL1 - 0.008, { speed: 0.025 });
            walk('leper', X.tentL2 + 0.012, { speed: 0.025 }); walk('stranger', X.hearth, { speed: 0.025 });
          }],
          [16.6, b => {
            say(b, [{ text: '「地不可永卖，因为地是我的；你们在我面前是客旅，是寄居的。', ref: '利未记 25:23', hold: 6.5 }]);
            walk('eleazar', X.altar + 0.022, { speed: 0.03 }); walk('ithamar', X.tabL - 0.03, { speed: 0.03 });
            rm('levi1'); rm('levi2');
            W.set('levHoly', 0.3);
          }],
        ]);
    ...
    // ── 26—27 时雨降下，旷野发青；我要在你们中间行走；西奈山的命令 ──
    {
      ...
      apply(c) {
        T(c, [
          [0, b => {
            ...
            W.goTo(0.5, 7, b.instant);
            sfx(b, 'rain');
          }],
          [1.5, b => {
            S.green = 1;
            W.set('bare', 0.22, b.instant); W.set('bloom', 0.9, b.instant); W.set('herbs', 1, b.instant); W.set('trees', 0.3, b.instant);
          }],
          [6.4, b => { W.set('rain', 0, b.instant); W.set('storm', 0, b.instant); W.set('clouds', 0.4, b.instant); W.goTo(0.72, 22, b.instant); }],
          [7.4, b => { W.set('levTents', 1, b.instant); W.set('levGlory', 0.8, b.instant); people('stand'); sfx(b, 'angel', { soft: true }); }],
          [11, () => { people('gaze'); }],
          [15.4, b => {
            say(b, [{ text: '地上所有的，无论是地上的种子是树上的果子，<br>十分之一是耶和华的，是归给耶和华为圣的。', ref: '利未记 27:30', hold: 6.5 }]);
          }],
          [19.5, () => { people('stand'); W.set('levGlory', 0.55); }],
  ...
  // 禧年的角声：三次，一次比一次远——声音化作贴着大地荡开的光环
  function blast(b, i) {
    if (b.instant) return;
    const pts = [];
    for (const id of ['eleazar', 'ithamar', 'levi1', 'levi2']) { const h = headOf(id, 0.9); if (h) pts.push(h); }
    fl(b, { type: 'waves', pts, R: Math.hypot(W.w, W.h) * (0.45 + 0.2 * i), dur: 3.6 });
