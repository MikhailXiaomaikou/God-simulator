 ...
 * 良人在本卷里从不画成人形：他是一只光的羚羊。夜里他从雪山上蹿山越岭而来，站在墙壁后从窗棂往里观看，
 * 「起来，与我同去！」——天将亮。「冬天已往，雨水止住过去了」（本卷的签名之景）：雨停云开，日出，
 * 遍地百花开放，树发芽开花，葡萄园吐叶放香，斑鸠飞来，百合满谷。磐石穴中的鸽子飞出；小狐狸被赶出葡萄园；
 * 日影飞去，羚羊转回比特山。夜里她在城中寻找，看守的人的火把；寻见了。黎明，旷野上来烟柱：所罗门的轿与勇士，
 ...
 * 却不能息灭（第二幅签名之景）。末了，天将亮：「求你使我也得听见」——羚羊跃上香草山，立在日出的金光里。
 *
...
(function (GS) {
  ...
  W.defineLevel('sgGz', 'exp', 1.1);       // 羚羊（良人）显现
  ...
  function sprites() {
    ...
    try {
      SP = {
        ...
        puff: radial([236, 230, 220], 1, 0.55), smoke: radial([196, 184, 172], 1, 0.6), dew: radial([220, 240, 255], 1, 0.3),
      };
    } catch (e) { SP = null; }
    return SP;
  }
  ...
  // 晨光如展开的旌旗（6:10）：日出之处向上展开的一束束柔光（金与玫瑰色相间，边缘柔和）；只在天亮时显出
  function drawRays(ctx) {
    ...
    const R = Math.hypot(W.w, W.h) * 0.95;
    const A = k * (0.35 + 0.65 * W.dusk);
    if (A < 0.01) return;
    ctx.globalCompositeOperation = 'lighter';
    const N = 8;
    for (let c = 0; c < 2; c++) {
      const g = ctx.createRadialGradient(ox, oy, R * 0.04, ox, oy, R);
      const col = c ? [255, 186, 170] : [255, 226, 176];
      g.addColorStop(0, rgba(col, 0.2)); g.addColorStop(0.3, rgba(col, 0.1)); g.addColorStop(0.7, rgba(col, 0.03)); g.addColorStop(1, rgba(col, 0));
      ctx.fillStyle = g;
      // 三层由宽到窄叠起来，边缘就柔了
      for (const [wk, al] of [[1.9, 0.3], [1.2, 0.45], [0.6, 0.7]]) {
        ctx.globalAlpha = A * al;
        ctx.beginPath();
        for (let i = c; i < N; i += 2) {
          const a = -Math.PI * (0.1 + 0.8 * (i + 0.5) / N) + Math.sin(W.t * 0.3 + i * 1.7) * 0.025;
          const wA = (0.016 + 0.006 * Math.sin(W.t * 0.5 + i * 2.3)) * wk;
          const bend = Math.sin(W.t * 0.45 + i) * 0.035;
          const a0 = a - wA, a1 = a + wA;
          ctx.moveTo(ox, oy);
          ctx.quadraticCurveTo(ox + Math.cos(a0 + bend) * R * 0.55, oy + Math.sin(a0 + bend) * R * 0.55, ox + Math.cos(a0) * R, oy + Math.sin(a0) * R);
          ctx.lineTo(ox + Math.cos(a1) * R, oy + Math.sin(a1) * R);
          ctx.quadraticCurveTo(ox + Math.cos(a1 + bend) * R * 0.55, oy + Math.sin(a1 + bend) * R * 0.55, ox, oy);
          ctx.closePath();
        }
        ctx.fill();
      }
  ...
  function drawTree(ctx, m, x, y, H, l, seed, o) {
    ...
    const sway = W.wind * 0.02 + lv('gale') * 0.06 * Math.sin(W.t * 1.9 + seed) + lv('gale') * 0.03;
  ...
  function drawGarden(ctx) {
    ...
    for (let i = 0; i < LILIES.length; i++) {
      ...
      lilyAt(p, xf * W.w + Math.sin(W.t * 1.3 + q.ph) * lv('gale') * 1.5 * s, baseY(2, xf, q.v) + 1, s, k);
      any = true;
    }
    // 荆棘内的第一朵
    const k1 = lv('sgLily1');
  ...
  function drawMeadow(ctx) {
    ...
    for (let i = 0; i < MEADOW.length && i < n + 1; i++) {
      ...
      const sw = (W.wind * 0.6 + gale * 1.6 * Math.sin(W.t * 2.4 + q.ph)) * s;
  ...
  function drawTrack(ctx) {
    ...
    for (const q of TRACK) {
      ...
      for (const d of [-1, 1]) { const px = x + d * 1.5 * s, py = y + d * 0.45 * s; p.moveTo(px + 1.25 * s, py); p.ellipse(px, py, 1.25 * s, 0.7 * s, 0, 0, TAU); }
    }
    ...
    if (SP) {
      for (const q of TRACK) {
        if (q.u > k) break;
        const xf = LX(q.f);
        glowAt(ctx, SP.gold, xf * W.w, baseY(2, xf, q.v), 5 * s, a * 0.35 * (0.7 + 0.3 * Math.sin(W.t * 2.2 + q.u * 9)));
      }
      ctx.globalAlpha = 1;
    }
    ctx.globalCompositeOperation = 'source-over';
  ...
  // 泉与溪水（4:12，4:15）
  const STREAM = [[0.42, 0.2], [0.385, 0.28], [0.33, 0.34], [0.26, 0.39], [0.19, 0.45], [0.12, 0.5], [0.05, 0.56], [-0.03, 0.64]];
  function streamPts() {
  ...
  function drawStream(ctx) {
    ...
    for (let i = 1; i < pts.length && acc < reach; i++) {
      ...
      const u = Math.min(1, (reach - acc) / seg[i - 1]);
      const bx = lerp(a[0], b[0], u), by = lerp(a[1], b[1], u);
      const s = LS(2) * (1 + 0.9 * lerp(a[2], b[2], u * 0.5));
      ctx.strokeStyle = rgba(water, 0.95);
      ctx.lineWidth = Math.max(1, 3.4 * s);
      ctx.beginPath(); ctx.moveTo(a[0], a[1]); ctx.lineTo(bx, by); ctx.stroke();
      ctx.strokeStyle = rgba(mix(water, [240, 248, 255], 0.55), 0.55 * (0.4 + 0.6 * W.daylight) + 0.25 * nightK());
      ctx.lineWidth = Math.max(0.5, 1 * s);
      ctx.beginPath(); ctx.moveTo(a[0], a[1] - 0.6 * s); ctx.lineTo(bx, by - 0.6 * s); ctx.stroke();
      acc += seg[i - 1];
    }
    // 水上的闪光
    ctx.fillStyle = U.rgba(255, 252, 236, 0.7 * (0.3 + 0.7 * W.daylight));
    let acc2 = 0;
    for (let i = 1; i < pts.length; i++) {
      const a = pts[i - 1], b = pts[i];
      for (let j = 0; j < 3; j++) {
        const u = (j + 0.5 + 0.5 * Math.sin(W.t * 1.7 + i * 3 + j)) / 3.5;
        if (acc2 + u * seg[i - 1] > reach) continue;
        const x = lerp(a[0], b[0], u), y = lerp(a[1], b[1], u), s = LS(2);
        if (Math.sin(W.t * 4 + i * 5.1 + j * 2.3) > 0.55) ctx.fillRect(x - 1.2 * s, y - 0.4 * s, 2.4 * s, 0.8 * s);
      }
      acc2 += seg[i - 1];
    }
  }
  ...
  function cachedNear(ctx) {
    const s = LS(2), wind = Math.round(W.wind * 16);
    // 园墙（后墙、左墙）与城
    ...
    drawHouse(ctx);
    // 树与葡萄园
    const tk = 't' + q40(lv('sgLeaf')) + ',' + q40(lv('sgBlos')) + ',' + q40(lv('sgFig')) + ',' + q40(lv('sgApple')) + ',' + q40(lv('sgPom')) + ',' + q40(lv('sgPomFr')) + ',' + q40(lv('sgVine')) + ',' + q40(lv('sgGrape')) + ',' + wind;
    ...
    drawStream(ctx);
    // 野花、百合与荆棘
    const mk = 'm' + Math.round(lv('sgMeadow') * 250) + ',' + Math.round(lv('sgLily') * 30) + ',' + q40(lv('sgLily1')) + ',' + q40(lv('sgThorn')) + ',' + wind;
  ...
  // 海边的磐石：烈焰燃在其上（8:6）
  function rockGeom() { const s = LS(2), xf = LX(F.rock), x = xf * W.w, y = gY(2, xf) + 3 * s; return { s, x, y, top: y - 11 * s }; }
  ...
  // 烈焰：几层火舌；大浪打来时低一低，随即更旺
  function drawFlame(ctx) {
    const k = lv('sgFlame');
    if (k < 0.01) return;
    sprites();
    const R = rockGeom(), s = R.s, x = R.x - 1 * s, y = R.top + 1 * s;
    const h = 24 * s * (0.75 + 0.25 * k) * (1 - 0.35 * flick) * (1 + 0.15 * lv('sgWaves'));
    ctx.globalCompositeOperation = 'lighter';
    glowAt(ctx, SP.warm, x, y - h * 0.45, h * 2.4, k * (0.35 + 0.45 * nightK() + 0.2 * lv('storm')));
    glowAt(ctx, SP.gold, x, y - h * 0.4, h * 1.1, k * 0.5);
    const T4 = [[0, 1, 'rgb(255,112,40)', 0.78], [-0.26, 0.7, 'rgb(255,156,60)', 0.72], [0.24, 0.74, 'rgb(255,136,52)', 0.72], [0, 0.5, 'rgb(255,240,190)', 0.92]];
    const lean = -lv('gale') * 0.35 + W.wind * 0.1;
    for (let i = 0; i < 4; i++) {
      const q = T4[i];
      const f = 0.82 + 0.12 * Math.sin(W.t * 13 + i) + 0.08 * Math.sin(W.t * 23.7 + i * 2);
      const H = h * q[1] * f, w = h * 0.24 * q[1];
      const sx = x + q[0] * h * 0.5 + Math.sin(W.t * 6 + i * 3) * h * 0.05;
      ctx.globalAlpha = k * q[3];
      ctx.fillStyle = q[2];
      ctx.beginPath();
      ctx.moveTo(sx - w, y);
      ctx.quadraticCurveTo(sx - w * 0.9 + lean * H * 0.2, y - H * 0.55, sx + lean * H * 0.5 + Math.sin(W.t * 8 + i) * w * 0.45, y - H);
      ctx.quadraticCurveTo(sx + w * 0.9 + lean * H * 0.2, y - H * 0.55, sx + w, y);
      ctx.closePath(); ctx.fill();
    }
    ctx.globalCompositeOperation = 'source-over';
    ctx.globalAlpha = 1;
  ...
  // ════════════════════════════════════════════════════════════
  function cleftGeom() { const s = LS(1), x = MX.rock * W.w, y = gY(1, MX.rock) + 2 * s; return { s, x, y, cx: x + 1.5 * s, cy: y - 12 * s }; }
  function drawCleftRock(ctx) {
    const { s, x, y } = cleftGeom();
    const H = 26 * s, Wd = 16 * s;
    const pts = [[-1.2, 0.05], [-1, -0.35], [-0.72, -0.62], [-0.45, -0.92], [-0.12, -1], [0.18, -0.9], [0.5, -0.78], [0.78, -0.5], [1.05, -0.22], [1.25, 0.05]];
    ctx.fillStyle = css([150, 138, 124], 1);
    ctx.beginPath();
    pts.forEach((p, i) => { const px = x + p[0] * Wd, py = y + p[1] * H; if (i) ctx.lineTo(px, py); else ctx.moveTo(px, py); });
    ctx.closePath(); ctx.fill();
    ctx.fillStyle = css([96, 88, 80], 1, 0.55);
    ctx.beginPath(); ctx.moveTo(x + 0.18 * Wd, y - 0.9 * H); ctx.lineTo(x + 0.5 * Wd, y - 0.78 * H); ctx.lineTo(x + 1.05 * Wd, y - 0.22 * H); ctx.lineTo(x + 1.25 * Wd, y + 0.05 * H); ctx.lineTo(x + 0.35 * Wd, y + 0.05 * H); ctx.closePath(); ctx.fill();
    // 陡岩的裂缝（隐密处）
    ctx.fillStyle = css([28, 24, 22], 1);
    ctx.beginPath();
    ctx.moveTo(x + 0.02 * Wd, y - 0.86 * H); ctx.lineTo(x + 0.2 * Wd, y - 0.6 * H); ctx.lineTo(x + 0.14 * Wd, y - 0.25 * H);
    ctx.lineTo(x + 0.02 * Wd, y - 0.28 * H); ctx.lineTo(x + 0.08 * Wd, y - 0.58 * H); ctx.closePath(); ctx.fill();
    ctx.strokeStyle = css([226, 214, 194], 1, 0.35 * dayA(), 0.2); ctx.lineWidth = Math.max(0.5, 0.7 * s);
    ctx.beginPath(); pts.slice(0, 6).forEach((p, i) => { const px = x + p[0] * Wd, py = y + p[1] * H; if (i) ctx.lineTo(px, py); else ctx.moveTo(px, py); }); ctx.stroke();
  }
  ...
  function drawSmoke(ctx) {
    const k = lv('sgSmoke');
    if (k < 0.01 || !SP) return;
    const s = LS(1), lx = litterX(), gx = lx * W.w, gy = gY(1, lx);
    const H = W.h * 0.2;
    for (let c = 0; c < 3; c++) {
      const bx = gx + (c - 1) * 14 * s + (c === 1 ? 0 : 0), by = gy - 14 * s;
      for (let i = 0; i < 14; i++) {
        const q = ((i / 14) + W.t * 0.045 + c * 0.31) % 1;
        const y = by - q * H, x = bx + Math.sin(q * 5 + W.t * 0.4 + c) * 5 * s * (0.4 + q) + W.wind * q * 18 * s;
        const r = (4 + 14 * q) * s;
        glowAt(ctx, SP.smoke, x, y, r, k * (1 - q) * (0.3 + 0.25 * W.daylight));
        glowAt(ctx, SP.puff, x - r * 0.15, y - r * 0.2, r * 0.6, k * (1 - q) * 0.25);
      }
    }
    ctx.globalAlpha = 1;
  }
  function drawLitter(ctx) {
    const k = lv('sgLitter');
    if (k < 0.01) return;
    const s = LS(1) * 1.5, lx = litterX(), x = lx * W.w, y = gY(1, lx) - 13.5 * s;
  ...
  function dovePerchXY() {
    if (S.dove === 'apple') { const b = appleBranch(); return [b[0], b[1], LS(2) * 1.05, 2]; }
    const c = cleftGeom(); return [c.cx, c.cy, LS(1) * 1.1, 1];
  }
  function drawDoveAt(ctx, layer) {
    if (FXL.some(e => e.type === 'dove')) return;
    const p = dovePerchXY();
    if (p[3] !== layer) return;
    if (SP) { ctx.globalCompositeOperation = 'lighter'; glowAt(ctx, SP.white, p[0], p[1] - 2 * p[2], 9 * p[2], 0.2 + 0.4 * nightK()); ctx.globalCompositeOperation = 'source-over'; ctx.globalAlpha = 1; }
  ...
  function foxPath(i, t) {
    // 自她家后面钻进葡萄园，东窜西窜，被赶出去
    const K = [
      [[0, 0.84, 0.08], [2.4, 0.7, 0.12], [4.2, 0.64, 0.2], [5.6, 0.7, 0.16], [8.5, 1.08, 0.1]],
      [[0.4, 0.85, 0.2], [2.8, 0.66, 0.3], [4.4, 0.72, 0.36], [6, 0.76, 0.3], [8.8, 1.1, 0.24]],
      [[0.8, 0.83, 0.34], [3.1, 0.61, 0.42], [4.8, 0.66, 0.46], [6.3, 0.74, 0.44], [9, 1.1, 0.4]],
    ][i];
    if (t <= K[0][0]) return null;
    for (let j = 1; j < K.length; j++) {
      if (t <= K[j][0]) {
        const u = ease((t - K[j - 1][0]) / (K[j][0] - K[j - 1][0]));
        return { f: lerp(K[j - 1][1], K[j][1], u), v: lerp(K[j - 1][2], K[j][2], u), face: K[j][1] >= K[j - 1][1] ? 1 : -1 };
      }
    }
    return null;
  }
  ...
  function gzPt(p) {
    const xf = p.x, l = p.l;
    return { x: xf * W.w, y: baseY(l, xf, p.v || 0) + (l === 2 ? 1.5 * LS(2) : 0.5), s: GZ_K * LS(l) * (1 + 0.35 * (p.v || 0)) * (l === 3 ? 2.5 : l === 0 ? 1.4 : 1) };
  }
  // 蹿山越岭：同一层上一跳一跳，层与层之间一大跃（越过海湾）
  function gzGo(b, pts, o) {
    o = o || {};
    const hops = [];
    let cur = { x: S.gz.x, l: S.gz.l, v: S.gz.v || 0 };
    ...
    for (const p of pts) {
      if (p.l === cur.l) {
        ...
        for (let i = 0; i < n; i++) {
          const a = { x: lerp(cur.x, p.x, i / n), l: p.l, v: lerp(cur.v, p.v || 0, i / n) };
          const bb = { x: lerp(cur.x, p.x, (i + 1) / n), l: p.l, v: lerp(cur.v, p.v || 0, (i + 1) / n) };
        ...
        lastFace = fc;
        hops.push({ a: cur, b: { x: p.x, l: p.l, v: p.v || 0 }, t0: t, t1: t + dur, big: true, face: fc });
        t += dur;
      }
      cur = { x: p.x, l: p.l, v: p.v || 0 };
    }
    ...
    if (!b.instant) sfx(b, 'wings', { soft: true });
  }
  ...
  function gzNow() {
    ...
    const P = gzPt(S.gz);
    const mode = S.gz.pose === 'graze' ? 2 : S.gz.pose === 'lie' ? 3 : 0;
    return { x: P.x, y: P.y, s: P.s, face: S.gz.face, mode, ph: 0, lay: S.gz.l, a: vis };
  }
  // 羚羊的身形：面向 +x，原点在四蹄之间的地面；单位像素 / s（角高约 40）→ { legs, body, horn }（Path2D）
  ...
  function drawGz(ctx, layer) {
    ...
    sprites();
    const nk = nightK();
    const col = mix([250, 232, 196], [255, 246, 226], nk);
    // 光晕；在香草山上时，山顶另有一大片金光（8:14）
    if (SP) {
      ctx.globalCompositeOperation = 'lighter';
      glowAt(ctx, SP.gold, g.x, g.y - 20 * g.s, 36 * g.s * (0.8 + 0.5 * nk), g.a * (0.28 + 0.5 * nk) * (layer === 3 ? 0.7 : 1));
      const sp = layer === 3 && !g.run ? lv('sgSpice') : 0;
      if (sp > 0.02) {
        const br = 0.85 + 0.15 * Math.sin(W.t * 0.9);
        glowAt(ctx, SP.rose, g.x, g.y - 12 * g.s, M() * 0.16, g.a * sp * 0.22 * br, 0.7);
        glowAt(ctx, SP.gold, g.x, g.y - 18 * g.s, M() * 0.06, g.a * sp * 0.45 * br);
      }
      ctx.globalCompositeOperation = 'source-over';
    }
    const P = gazellePaths(g);
    ctx.save();
    ctx.translate(g.x, g.y);
    ctx.scale(g.face * g.s, g.s);
    ctx.rotate(P.pitch);
    ctx.lineCap = 'round'; ctx.lineJoin = 'round';
    // 一道暗的外缘：在亮的天、亮的山前也分得出它的身形
    const edge = clamp(W.daylight * 0.9 + (layer === 3 ? 0.4 : 0), 0, 1) * 0.5;
    if (edge > 0.02) {
      ctx.globalAlpha = g.a * edge;
      ctx.strokeStyle = 'rgb(96,66,50)';
      ctx.lineWidth = 2.6; ctx.stroke(P.legs);
      ctx.lineWidth = 1.2; ctx.stroke(P.body);
      ctx.lineWidth = 2.1; ctx.stroke(P.horn);
    }
    ctx.globalAlpha = g.a;
    ctx.strokeStyle = rgba(col, 1);
    ctx.fillStyle = rgba(col, 1);
    ctx.lineWidth = 1.5; ctx.stroke(P.legs);
    ctx.fill(P.body);
    ctx.lineWidth = 1.05; ctx.stroke(P.horn);
    ctx.restore();
    ctx.globalAlpha = 1;
  }
  ...
  function petals(dir, k) {
    ...
    for (let i = 0; i < 46 * k; i++) {
      emit({ x: xy[0] + (Math.random() - 0.5) * H * 0.7, y: xy[1] - H * (0.45 + Math.random() * 0.45), vx: dir * (40 + Math.random() * 90) * SU() * k, vy: -10 + Math.random() * 25,
        max: 3 + Math.random() * 3, c: Math.random() < 0.6 ? [252, 232, 236] : [240, 186, 204], s: (0.9 + Math.random() * 0.9) * LS(2), kind: 'petal', rot: Math.random() * TAU, vr: (Math.random() - 0.5) * 6, pass: 'air' });
    }
  }
  function drawPT(ctx, pass) {
    ...
    for (const p of PT) {
      ...
      const u = p.life / p.max, a = Math.min(1, u * 5) * (1 - u);
      if (a <= 0.01) continue;
      if (p.kind === 'petal') {
        ctx.globalCompositeOperation = 'source-over';
        ctx.globalAlpha = a;
        ctx.fillStyle = rgba(W.shade(p.c, 0, 0.2), 1);
        ctx.beginPath(); ctx.ellipse(p.x, p.y, 1.4 * p.s, 0.8 * p.s, p.rot, 0, TAU); ctx.fill();
      } else if (p.kind === 'spray') {
        ctx.globalCompositeOperation = 'source-over';
        ctx.globalAlpha = a * 0.85;
        ctx.fillStyle = 'rgb(236,244,250)';
        ctx.beginPath(); ctx.arc(p.x, p.y, p.s, 0, TAU); ctx.fill();
      } else {
        if (!add) { ctx.globalCompositeOperation = 'lighter'; add = true; }
  ...
  function drawFXL(ctx) {
    ...
    for (const e of FXL) {
      ...
      if (e.type === 'dove') {
        ...
        const s = lerp(A[2], B[2], k);
        if (SP) { ctx.globalCompositeOperation = 'lighter'; glowAt(ctx, SP.white, x, y - 2 * s, 12 * s, 0.35 + 0.4 * nightK()); ctx.globalCompositeOperation = 'source-over'; }
        ...
        for (let i = 0; i < 2; i++) {
          ...
          const ax = W.w * 1.06, ay = W.h * (0.3 + i * 0.04);
          const bx = G.x + (4 + i * 4.5) * G.s, by = G.y - G.h - 2 * G.s;
          const x = lerp(ax, bx, k) + Math.sin(ui * 9 + i) * W.w * 0.03 * (1 - k);
          const y = lerp(ay, by, k) - Math.sin(Math.PI * k) * W.h * 0.08;
          drawDove(ctx, x, y, G.s * 0.95, bx < ax ? -1 : 1, (t * 3.1 + i * 0.4) % 1, ui > 0.98, [150 + i * 10, 132 + i * 8, 118 + i * 6], 1);
        }
      } else if (e.type === 'foxes') {
        for (let i = 0; i < 3; i++) {
          const q = foxPath(i, t - e.t0);
          if (!q) continue;
          const xf = LX(q.f), s = LS(2) * (1 + 0.35 * q.v) * 0.95;
          const a = q.f > 1 ? 0 : q.f > 0.96 ? (1 - q.f) / 0.04 : 1;
          drawFox(ctx, xf * W.w, baseY(2, xf, q.v) + 1, s, q.face, (t * 3.2 + i * 0.3) % 1, a);
        }
      } else if (e.type === 'spring') {
        // 春的光自东（左）向西扫过全地
        if (!SP) continue;
        const bx = lerp(-0.2, 1.2, ease(u)) * W.w;
        const a = Math.sin(Math.PI * u) * 0.35;
        ctx.globalCompositeOperation = 'lighter';
        glowAt(ctx, SP.gold, bx, W.h * 0.8, W.w * 0.28, a, 0.55);
        ctx.globalCompositeOperation = 'source-over';
      } else if (e.type === 'crown') {
        if (!SP) continue;
        const lx = litterX(), s = LS(1), x = lx * W.w, y = gY(1, lx) - 44 * s;
        const a = Math.sin(Math.PI * u);
        ctx.globalCompositeOperation = 'lighter';
        glowAt(ctx, SP.gold, x, y, 22 * s, a * 0.8);
        ctx.globalAlpha = a; ctx.strokeStyle = 'rgba(255,240,200,0.9)'; ctx.lineWidth = Math.max(0.6, 0.8 * s);
        ctx.beginPath(); ctx.moveTo(x - 9 * s, y); ctx.lineTo(x + 9 * s, y); ctx.moveTo(x, y - 9 * s); ctx.lineTo(x, y + 9 * s); ctx.stroke();
        ctx.globalCompositeOperation = 'source-over';
        ...
        const g = gzNow();
        if (g && SP) { ctx.globalCompositeOperation = 'lighter'; for (let i = 0; i < 7; i++) { const a = Math.sin(t * 5 + i * 2.1) * 0.5 + 0.5; glowAt(ctx, SP.dew, g.x + (i - 3) * 2.6 * g.s * g.face, g.y - (26 + (i % 3) * 5) * g.s, 3 * g.s, a * Math.sin(Math.PI * u)); } ctx.globalCompositeOperation = 'source-over'; }
      }
    }
    ctx.globalAlpha = 1;
  }
  ...
  function drawWaves(ctx) {
    ...
    const foam = 'rgba(236,244,250,' + (0.55 * k * (0.5 + 0.5 * W.daylight) + 0.2 * k).toFixed(3) + ')';
    const N = 5;
    for (let i = 0; i < N; i++) {
      ...
      const Y = x => { const ph = ((x - off) / wl) % 1, f = ph < 0 ? ph + 1 : ph; const c = Math.pow(Math.max(0, Math.sin(f * Math.PI)), 3); return yb - A * (c * 1.6 - 0.3); };
      ctx.beginPath();
      ctx.moveTo(-10, y1);
      for (let x = -10; x <= W.w * 0.75; x += 6) ctx.lineTo(x, Y(x));
      ctx.lineTo(W.w * 0.75, y1); ctx.closePath();
      ...
      ctx.strokeStyle = foam; ctx.lineWidth = Math.max(0.8, (1.2 + 2.4 * q) * SU());
      ctx.beginPath();
      let on = false;
      for (let x = -10; x <= W.w * 0.75; x += 5) {
        const ph = ((x - off) / wl) % 1, f = ph < 0 ? ph + 1 : ph;
        if (f > 0.35 && f < 0.62) { if (!on) { ctx.moveTo(x, Y(x)); on = true; } else ctx.lineTo(x, Y(x)); } else on = false;
      }
      ctx.stroke();
    }
  }
  ...
  const SCENE = {
    ...
    resize() {
      ...
      // 手机转了方向：近地的逻辑位置映到画面的方式变了——近地上的人、羊与羚羊一并挪到新的位置
      const p = port();
    ...
    update(dt) {
      ...
      if (RUN) {
        ...
        if (trailT <= 0) {
          trailT = 0.06;
          const g = gzNow();
          if (g && fx()) fx().sparkle(g.x - g.face * 6 * g.s, g.y - 16 * g.s, 1, [255, 234, 186], 4 * g.s, ['far', 'mid', 'near', 'sky'][g.lay]);
        }
      }
      // 园中的香气（4:16）
      const fr = lv('sgFrag');
      ...
      if (wv > 0.35) {
        sprayT -= dt;
        if (sprayT <= 0) {
          sprayT = 1.1 + Math.random() * 1.2;
          const R = rockGeom();
          flick = Math.min(1, 0.6 + Math.random() * 0.4);
          for (let i = 0; i < 70 * wv; i++) {
            emit({ x: R.x - 16 * R.s + Math.random() * 22 * R.s, y: R.y - Math.random() * 6 * R.s, vx: (-10 + Math.random() * 80) * SU(), vy: -(90 + Math.random() * 170) * SU() * wv,
              max: 1 + Math.random() * 0.9, s: (1 + Math.random() * 1.8) * SU(), kind: 'spray', pass: 'air' });
          }
          sfx({}, 'splash', { soft: true, x: 0.3 });
        }
      }
    },
    drawUnder(ctx, pass) {
      ...
      if (pass === 'mid') {
        ...
        if (calm()) drawCached(ctx, 'mid', 'mt' + q40(lv('sgLeaf')) + ',' + q40(lv('sgBlos')) + ',' + Math.round(W.wind * 16), [Math.floor(W.w * 0.53), Math.floor(top), Math.ceil(W.w * 0.98), Math.ceil(W.waterlineY(1) + 4)], g => drawMidTrees(g));
    ...
    draw(ctx, pass) {
      ...
      else if (pass === 'near') { drawFlame(ctx); drawDoveAt(ctx, 2); drawGz(ctx, 2); }
    },
    reset() { RUN = null; LIT = null; FXL.length = 0; PT.length = 0; for (const k in CACHE) delete CACHE[k]; },
    restore() { RUN = null; LIT = null; FXL.length = 0; PT.length = 0; flick = 0; },
    // 看完与恢复是否一致（走查工具比较它）
    ...
    pick(x, y, r) {
      ...
      const g = gzNow();
      if (g && g.a > 0.5) consider('良人', g.x, g.y - 22 * g.s);
      for (const t of TREES) { const xy = treeXY(t), H = treeH(t); consider(t.label, xy[0], xy[1] - H * 0.55); }
      { const xf = LX(F.palm); consider('棕树', xf * W.w, gY(2, xf) - 80 * LS(2)); }
      ...
      if (lv('sgLitter') > 0.5) { const lx = litterX(); consider('所罗门的轿', lx * W.w, gY(1, lx) - 32 * LS(1)); }
  ...
  function drawAir(ctx) {
    ...
    if (SP && (lv('sgWin') > 0.02 || (lv('sgLamp') > 0.02 && nk > 0.1) || lv('sgDoor') > 0.02)) {
      ...
      glowAt(ctx, SP.warm, G.dx, G.y - 6 * G.s, 22 * G.s, lv('sgDoor') * (0.25 + 0.5 * nk));
      ctx.globalCompositeOperation = 'source-over'; ctx.globalAlpha = 1;
    }
    // 园门口的光、羚羊的光在人身上
    drawFXL(ctx);
  ...
  // ════════════════════════════════════════════════════════════
  function setup() {
    // 冬天：灰云、细雨；地色暗淡，花隐去；树是光秃的（本卷自己画树，大地的树不长）
    W.set('bare', 0.34, true); W.set('bloom', 0, true); W.set('storm', 0.3, true); W.set('rain', 0.3, true); W.set('gale', 0.08, true);
    const L = { deep: 1, light: 1, gather: 1, dayNight: 1, vault: 1, clouds: 0.78, land: 1, grass: 0.62, herbs: 0.28, trees: 0, lights: 1, moon: 1, stars: 1, life: 1, good: 0, sabbath: 0, given: 1,
    ...
    // 本卷的局部状态
    S = fresh();
    RUN = null; LIT = null; FXL.length = 0; PT.length = 0; flick = 0; GUST = -1;
    lastPort = port();
    MODELS = null;
    const c = C();
    c.clear({ fade: false });
    ...
    beast('kid1', { kind: 'kid', x: LX(0.61), label: '山羊羔', facing: -1, pose: 'graze' });
    beast('kid2', { kind: 'kid', x: LX(0.7), label: '山羊羔', facing: -1, pose: 'graze' });
    herd('flock', { kind: 'sheep', n: 7, x0: LX(0.0), x1: LX(0.2), label: '羊群', pose: 'graze' }).forEach((m, i) => { m.v = 0.2 + ((i * 0.618) % 1) * 0.25; });
    crowd('shepherds', { n: 2, x0: LX(0.07), x1: LX(0.15), label: '牧人' }, man(ROBE.shepherd, 'staff'));
  ...
  const STAGES = [
      ...
      apply(c) {
        T(c, [
          [0, b => {
            ...
            W.set('rain', 0.14, b.instant); W.set('storm', 0.18, b.instant); W.set('clouds', 0.66, b.instant);
          ...
          [3.4, () => {
            ...
            walk('bride', LX(0.21), { speed: 0.019, pose: 'stand' });
          }],
          [7.5, b => { cpose('flock', 'lie'); cpose('shepherds', 'sit'); sfx(b, 'bleat', { soft: true, x: 0.55 }); }],
          [15, b => {
            follow('kid1', null); follow('kid2', null);
            walk('kid1', LX(0.18), { speed: 0.02, pose: 'graze' }); walk('kid2', LX(0.245), { speed: 0.02, pose: 'graze' });
    ...
    // ── 2:2 好像百合花在荆棘内：荆棘中开出第一朵百合；黄昏，她回家点灯 ─────────
    {
      ...
      apply(c) {
        T(c, [
          ...
          [9.5, b => { W.goTo(0.745, 10, b.instant); W.set('rain', 0.22, b.instant); W.set('storm', 0.26, b.instant); }],
    ...
    // ── 2:10 起来，与我同去：夜里光的羚羊蹿山越岭而来，站在墙壁后，从窗棂往里观看 ─────
    {
      ...
      apply(c) {
        T(c, [
          [0, b => {
            W.goTo(0.205, 15, b.instant); W.set('rain', 0.08, b.instant); W.set('storm', 0.14, b.instant); W.set('clouds', 0.5, b.instant);
          ...
          [1.6, b => { gzGo(b, [Rg(0.845), Fa(0.8), Fa(0.74), Mi(0.72), Mi(0.62), Nr(0.57), Nr(0.742)], { face: 1 }); }],
          [10.4, b => { W.set('sgWin', 1, b.instant); sfx(b, 'chime', { soft: true }); }],
          [15, b => {
            W.set('sgDoor', 1, b.instant);
            add('bride', { label: '书拉密女', sex: 'f', x: LX(0.786), facing: -1, pose: 'stand', robe: ROBE.bride, accent: [226, 178, 156], hair: 'veil', glow: 0.4, prop: null });
    ...
    // ── 2:14 我的鸽子啊，你在磐石穴中：鸽子飞出；小狐狸被赶出葡萄园；日影飞去，羚羊转回比特山 ─────
    {
      ...
      apply(c) {
        T(c, [
          [0, b => {
            ...
            sfx(b, 'dove');
            if (!b.instant && fx()) { const g = cleftGeom(); fx().ring(g.cx, g.cy, [255, 250, 236], M() * 0.08, 1.8, 1.2); }
          }],
          [1.2, b => {
            if (!b.instant) FXL.push({ type: 'dove', t0: W.t, dur: 4.6, from: () => { const g = cleftGeom(); return [g.cx, g.cy, LS(1) * 1.1]; }, to: () => { const p = appleBranch(); return [p[0], p[1], LS(2) * 1.05]; } });
            S.dove = 'apple';
          ...
          [9.3, b => {
            if (!b.instant) FXL.push({ type: 'foxes', t0: W.t, dur: 9.4 });
            crowd('keepers', { n: 2, x0: LX(0.83), x1: LX(0.86), label: '同母的弟兄' }, man([ROBE.keeper, [100, 84, 70]]));
            cwalk('keepers', LX(0.66), LX(0.72), { speed: 0.03, pose: 'point' });
            face('bride', 1);
          }],
          [11.5, b => { gzGo(b, [Nr(0.5, 0.1), Nr(0.64, 0.16)], { face: 1 }); }],
          [15.2, () => { cwalk('keepers', LX(0.78), LX(0.83), { speed: 0.025, pose: 'stand' }); cface('keepers', -1); }],
          [17.2, b => { W.goTo(0.745, 9, b.instant); W.set('clouds', 0.36, b.instant); }],
          [18.4, b => { gzGo(b, [Nr(0.72, 0.06), Mi(0.74), Fa(0.82), Rg(PK.bether)], { face: -1, delay: 0.2 }); sfx(b, 'harp', { soft: true }); }],
          [21, () => { pose('bride', 'gaze'); face('bride', 1); }],
        ]);
      },
    },

    // ── 3 夜里寻找；寻见了；黎明，旷野上来烟柱：所罗门的轿，锡安的众女子出城观看 ─────
    {
      kind: 'cmd', utter: '锡安的众女子啊，你们出去观看所罗门王！', cmd: 'open 城门 && watch 所罗门的轿 --crowned', ref: '3:11',
      verse: [
        { text: '我夜间躺卧在床上，寻找我心所爱的；我寻找他，却寻不见。', ref: '雅歌 3:1', hold: 5.5 },
      ...
      apply(c) {
        T(c, [
          [0, b => {
            W.goTo(0.02, 5, b.instant); W.set('sgLamp', 1, b.instant);
            crm('keepers'); crm('shepherds'); crm('flock'); rm('kid1'); rm('kid2');
            W.set('sgTents', 0, b.instant);
          }],
          [0.8, () => { hold('bride', 'torch'); pose('bride', 'stand'); walk('bride', LX(0.875), { speed: 0.042, pose: 'stand' }); }],
          [2, () => {
            crowd('watch', { n: 2, x0: LX(0.905), x1: LX(0.96), label: '看守的人' }, man([ROBE.watch, [84, 76, 90]], 'torch'));
            cface('watch', -1);
          }],
          [7, b => { face('bride', 1); pose('bride', 'point'); sfx(b, 'crowd', { soft: true, x: 0.85 }); }],
          [9.2, b => { cpose('watch', 'point'); gzGo(b, [Fa(0.84), Mi(0.78), Nr(0.585)], { face: 1 }); }],
          [10.2, () => { pose('bride', 'stand'); walk('bride', LX(0.615), { speed: 0.05, pose: 'stand' }); face('bride', -1); }],
          [15, b => {
            W.goTo(0.275, 9, b.instant);
            crm('watch');
            W.set('sgSmoke', 1, b.instant); W.set('sgLitter', 1, b.instant);
          ...
          [19, b => { W.set('sgLamp', 0.2, b.instant); hold('bride', null); }],
          [23.4, b => {
          ...
          [27.2, b => { cface('daughters', -1); cpose('daughters', 'raise'); if (!b.instant) FXL.push({ type: 'crown', t0: W.t, dur: 2.6 }); sfx(b, 'harp'); }],
        ]);
      },
    },
      ...
      apply(c) {
        T(c, [
          [0, b => {
            ...
            W.set('sgSmoke', 0, b.instant); W.set('sgLitter', 0, b.instant);
            crm('guard');
    ...
    // ── 4:16 北风啊，兴起！南风啊，吹来！香气发出来；良人进入自己的园中（5:1）─────
    {
      ...
      apply(c) {
        T(c, [
          ...
          [14.2, b => { W.set('sgGate', 0, b.instant); gzPose('graze'); }],
        ]);
      },
    },
    ...
    // ── 5:2 求你给我开门：夜里敲门；门开了，他已转身走了；「他的形状如黎巴嫩」 ─────
    {
      ...
      verse: [
        { text: '我身睡卧，我心却醒。这是我良人的声音；他敲门说：<br>我的妹子，我的佳偶，我的鸽子，我的完全人，求你给我开门；因我的头满了露水，我的头发被夜露滴湿。', ref: '雅歌 5:2', hold: 9 },
        { text: '我给我的良人开了门；我的良人却已转身走了。……<br>我寻找他，竟寻不见；我呼叫他，他却不回答。', ref: '雅歌 5:6', hold: 7 },
        { text: '我的良人白而且红，超乎万人之上。……他的形状如黎巴嫩，且佳美如香柏树。<br>……他全然可爱。耶路撒冷的众女子啊，这是我的良人；这是我的朋友。', ref: '雅歌 5:10–16', hold: 9 },
      ],
      apply(c) {
        T(c, [
          [0, b => {
          ...
          [11.4, b => {
            W.set('sgDoor', 1, b.instant);
            add('bride', { label: '书拉密女', sex: 'f', x: LX(0.786), facing: -1, pose: 'stand', robe: ROBE.bride, accent: [226, 178, 156], hair: 'veil', glow: 0.4, prop: 'torch' });
            sfx(b, 'gate', { soft: true });
          }],
    ...
    // ── 6:13 回来，回来，书拉密女：黎明如展开的旌旗；二人往葡萄园去（7:10–12）─────
    {
      ...
      apply(c) {
        T(c, [
          ...
          [3, () => { hold('bride', null); pose('bride', 'gaze'); face('bride', -1); }],
          [7.8, b => {
      ...
      apply(c) {
        T(c, [
          ...
          [6.8, b => {
            ...
            gzPose('lie', 1); face('bride', -1); pose('bride', 'stand');
            sfx(b, 'seal');
    ...
    // ── 8:13 求你使我也得听见：她的声音自园中升起；「求你快来！」——羚羊跃上香草山 ─────
    {
      kind: 'call', utter: '同伴都要听你的声音，求你使我也得听见', cmd: 'listen 你的声音 && await 羚羊 --on 香草山', ref: '8:13',
      verse: [
      ...
      apply(c) {
        T(c, [
          [0, b => {
            W.goTo(0.246, 14, b.instant); W.set('sgSeal', 0.35, b.instant); W.set('sgWaves', 0, b.instant);
  ...
  GS.book.act({
    ...
    outro: 20,
    intro: [
      { text: '所罗门的歌，是歌中的雅歌。', ref: '雅歌 1:1', hold: 5 },
      { text: '耶路撒冷的众女子啊，我虽然黑，却是秀美，如同基达的帐棚，好像所罗门的幔子。<br>不要因日头把我晒黑了就轻看我……他们使我看守葡萄园；我自己的葡萄园却没有看守。', ref: '雅歌 1:5–6', hold: 9.5 },
    ],
    ...
    behold: {
      ...
      '棕树': { text: '我说：我要上这棕树，抓住枝子。', ref: '雅歌 7:8' },
