...
(function (GS) {
  ...
  const X = {
    ...
    // 歌珊
    goshen0: 0.716, goshen1: 0.8, tentG1: 0.742, tentG2: 0.782, bed: 0.756, coffin: 0.74,
  };
  const ROBE = {
  ...
  const BRO_CN = { reuben: '吕便', simeon: '西缅', levi: '利未', judah: '犹大', dan: '但', naphtali: '拿弗他利', gad: '迦得', asher: '亚设', issachar: '以萨迦', zebulun: '西布伦' };
  ...
  // 全书的末一幅：架子高些、棺身是贴金的木色，一道柔光自上而下停在棺上，黄昏与夜里都认得出
  function drawCoffin(ctx, p) {
    const l = p.layer, s = LS(l) * p.size, x = p.x * W.w, y = gY(l, p.x) + 2.5 * s, L = 30 * s, legH = 6 * s;
    const nk = nightK();
    ctx.globalAlpha = p.a;
    // 地上的影
    ctx.fillStyle = U.rgba(12, 8, 6, 0.28);
    ctx.beginPath(); ctx.ellipse(x, y, L * 0.6, 2.2 * s, 0, 0, TAU); ctx.fill();
    // 架
    ctx.strokeStyle = css([92, 66, 42], l); ctx.lineWidth = Math.max(0.9, 1.3 * s); ctx.lineCap = 'round';
    ctx.beginPath();
    ctx.moveTo(x - L * 0.5, y - legH); ctx.lineTo(x + L * 0.52, y - legH);
    for (const q of [-0.42, -0.36, 0.38, 0.44]) { ctx.moveTo(x + q * L, y - legH); ctx.lineTo(x + q * L, y); }
    ctx.moveTo(x - 0.39 * L, y - legH * 0.45); ctx.lineTo(x + 0.41 * L, y - legH * 0.45);
    ctx.stroke();
    // 人形的棺：脚在左，头在右（向着迦南）
    const b = y - legH - 0.8 * s;
    const pts = [[-0.5, 0], [-0.52, -3.2], [-0.4, -4.4], [-0.1, -4.0], [0.16, -4.8], [0.3, -5.2], [0.38, -6.6], [0.48, -6.8], [0.54, -5.2], [0.53, 0]];
    const path = () => { ctx.beginPath(); pts.forEach((q, i) => { const X1 = x + q[0] * L, Y1 = b + q[1] * s; if (i) ctx.lineTo(X1, Y1); else ctx.moveTo(X1, Y1); }); ctx.closePath(); };
    ...
    // 迎光的边
    ctx.strokeStyle = css([255, 236, 190], l, 0.4 * dayA() + 0.35 * p.lit, 0.35); ctx.lineWidth = Math.max(0.6, 0.9 * s);
    ...
    if (p.lit > 0.01) {
      ...
      const breath = 0.85 + 0.15 * Math.sin(W.t * 0.55);
      const k = p.a * p.lit;
      // 自上而下的柔光
      const bw = L * 1.5, bh = Math.min(W.h * 0.45, L * 9);
      ctx.globalAlpha = Math.min(1, k * (0.3 + 0.25 * nk) * breath);
      ctx.drawImage(SP.beam, x + 0.05 * L - bw / 2, y - bh, bw, bh);
      glowSp(ctx, SP.warm, x + 0.05 * L, b - 3 * s, L * 1.9, k * 0.3 * breath * (0.5 + 0.5 * nk));
      glowSp(ctx, SP.gold, x + 0.05 * L, b - 3 * s, L * 1.2, k * 0.5 * br * (0.6 + 0.4 * nk));
      // 地上的一圈光
      ctx.globalAlpha = 1;
      ctx.fillStyle = U.rgba(255, 220, 160, k * 0.16 * br * (0.55 + 0.45 * nk));
      ctx.beginPath(); ctx.ellipse(x, y + 1 * s, L * 1.05, 5 * s, 0, 0, TAU); ctx.fill();
      ctx.fillStyle = U.rgba(255, 240, 206, k * 0.8 * br);
      ctx.beginPath(); ctx.arc(x + 0.12 * L, b - 4.9 * s, Math.max(0.9, 1.2 * s), 0, TAU); ctx.fill();
      ctx.globalCompositeOperation = 'source-over';
    }
    ctx.globalAlpha = 1;
  }
  ...
  // ── 几何的缓存：地的轮廓、风纹、河岸只随画面大小（与地的升起）而变；每帧只上色，不再逐点重算 ──
  let geoKey = '', GEO = null;
  function geo() {
  ...
  function drawDesert(ctx, l) {
    ...
    if (e > 0.01) {
      ...
      ctx.stroke(cachedPath('rim' + l, P2 => {
        for (let i = 0; i <= 30; i++) { const xf = lerp(0.3, x1, i / 30), y = gY(l, xf); if (i) P2.lineTo(xf * W.w, y + 0.8); else P2.moveTo(xf * W.w, y + 0.8); }
      }));
      if (l === 2) {
  ...
  function drawGoshen(ctx) {
    ...
    ctx.fillStyle = g;
    ctx.fill(landPath(2, a - 0.03, b + 0.03, 24));
    ...
    const B = cachedData('blades', () => {
      const out = [];
      for (let i = 0; i < 44; i++) {
        const xf = lerp(a, b, rt(i * 3 + 400)), v = rt(i * 3 + 401) * 0.8;
    ...
    for (let i = 0; i < nT; i++) {
      ...
      ctx.moveTo(q[0], q[1]); ctx.lineTo(q[0] + sw, q[1] - q[2]);
    }
    ctx.stroke();
  }
  ...
  function drawNile(ctx) {
    ...
    // 河宽与饥荒只在转换时变动：量化后缓存两岸的形状
    const wkQ = Math.round((0.25 + 0.75 * k) * (1 - 0.3 * fam) * 200) / 200, famQ = Math.round(fam * 50) / 50;
    const wk = wkQ;
    const s = LS(2);
    const edge = (mul, add) => cachedPath('nile:' + mul + ':' + add.toFixed(2) + ':' + wkQ, P2 => {
      const Lp = [], Rp = [];
    ...
    ctx.fillStyle = css(U.mixRGB([150, 124, 86], [190, 160, 116], famQ), 2);
    ctx.fill(edge(1.7, 6 * s * (1 + famQ)));
    ctx.globalAlpha = Math.min(1, k * 1.4);
    ctx.fillStyle = css(U.mixRGB([116, 96, 66], [150, 126, 90], famQ), 2);
    ctx.fill(edge(1.28, 2.5 * s * (1 + famQ * 2)));
    // 水
    ...
    // 芦荻（41:2）：根与高缓存，每帧只随风摆
    ctx.globalAlpha = Math.min(1, k * 1.4);
    const rc = U.mixRGB([88, 124, 62], [150, 132, 88], famQ);
    ctx.strokeStyle = css(rc, 2);
    ctx.lineWidth = Math.max(0.5, 0.8 * s);
    const nR = (W.quality || 1) < 0.75 ? 32 : 64;
    const R = cachedData('reeds:' + wkQ + ':' + famQ, () => {
      const out = [];
      for (let i = 0; i < 64; i++) {
        const t = 0.08 + 0.9 * rt(i * 3 + 900), side = i % 2 ? 1 : -1, p = nilePt(t), w = nileW(t, wk) * 0.62 + 2 * s;
        out.push([p[0] + p[2] * w * side, p[1] + p[3] * w * side, (3 + 6 * rt(i * 3 + 901)) * s * (0.4 + 1.1 * t) * (1 - 0.5 * famQ)]);
      }
      return out;
    });
    ctx.beginPath();
  ...
  const TRIBE = [];
  // 竖屏时经文在画面顶上：天上的星（十二支派、十四年）略往下放，不叫经文遮住
  const skyDY = () => (W.w < W.h * 0.75 ? 0.12 : 0);
  ...
  function drawTribes(ctx) {
    ...
    ctx.globalCompositeOperation = 'lighter';
    for (let i = 0; i < 12; i++) {
      const x = TRIBE[i][0] * W.w, y = (TRIBE[i][1] + skyDY()) * W.h, tw = 0.85 + 0.15 * Math.sin(W.t * 1.2 + i * 1.9);
  ...
  function drawCows(ctx, e) {
    const k0 = e.sp || 1, t = e.t * k0, D = e.dur * k0, s = LS(2) * 1.3, u = SU();   // sp：梦的快慢（经文短时，梦也紧凑些）
    SP || sprites();
    const A = smoothstep(0, 1, t) * (1 - smoothstep(D - 3.5, D, t));
    if (A < 0.01) return;
  ...
  function drawEars(ctx, e) {
    const k0 = e.sp || 1, t = e.t * k0, D = e.dur * k0, u = SU();
    SP || sprites();
    const A = smoothstep(0, 1, t) * (1 - smoothstep(D - 3.5, D, t));
    if (A < 0.01) return;
  ...
  function drawYears(ctx, e) {
    ...
    ctx.globalCompositeOperation = 'lighter';
    for (let i = 0; i < 14; i++) {
      const good = i < 7, x = lerp(0.3, 0.95, i / 13) * W.w, y = (0.24 - 0.09 * Math.sin(Math.PI * i / 13) + skyDY()) * W.h;
      ...
      switch (e.type) {
        ...
        case 'rise': {
          ...
          for (let i = 0; i < e.ids.length; i++) {
            ...
            const x = lerp(p[0], tg[0] * W.w, kk) + Math.sin(k * 5 + i) * 10 * u * (1 - k), y = lerp(p[1], (tg[1] + skyDY()) * W.h, kk);
  ...
  const HGT = { tent: 18, cave: 18, pit: 2, cloth: 3, altar: 8, bed: 7, bier: 18, palm: 44, pyramid: 30, house: 20, palace: 30, throne: 16, prison: 16, granary: 18, field: -8, dais: 8, coffin: 12, chariot: 14 };
  const SCENE = {
    init() { sprites(); },
    resize() {
      if (!isCur()) return;
    ...
    pick(x, y, r) {
      ...
      if (W.lv.jsTribes > 0.5) {
        for (let i = 0; i < 12; i++) {
          const sx = TRIBE[i][0] * W.w, sy = (TRIBE[i][1] + skyDY()) * W.h, d = Math.hypot(sx - x, sy - y);
  ...
  // 地上的走兽绕开主要人物与布景所在的几段（W.beastAvoid：近地层上宽度比例的区间；每卷开始时引擎会清空）
  ...
  function setup() {
    ...
    avoid([0.63, 0.77], [0.86, 0.96]);
  }

  // ════════════════════════════════════════════════════════════
  //  经文（和合本）——每一句话的经文不过四行（约半分钟），故事随经文一同说完
  // ════════════════════════════════════════════════════════════
  const INTRO = [
  ...
  const V2 = [
    { text: '后来他又做了一梦，也告诉他的哥哥们说：<br>「看哪，我又做了一梦，梦见太阳、月亮，与十一个星向我下拜。」', ref: '创世记 37:9', hold: 11 },
  ...
  const V3 = [
    { text: '……约瑟就去追赶他哥哥们，遇见他们在多坍。<br>他们远远地看见他，趁他还没有走到跟前，大家就同谋要害死他。', ref: '创世记 37:17–18', hold: 7.5 },
    { text: '吕便听见了，要救他脱离他们的手，说：「我们不可害他的性命」；<br>又说：「不可流他的血，可以把他丢在这野地的坑里，不可下手害他。」', ref: '创世记 37:21–22', hold: 7.5 },
    { text: '约瑟到了他哥哥们那里，他们就剥了他的外衣，就是他穿的那件彩衣，<br>把他丢在坑里；那坑是空的，里头没有水。', ref: '创世记 37:23–24', hold: 7.5 },
    { text: '有些米甸的商人从那里经过，哥哥们就把约瑟从坑里拉上来，<br>讲定二十舍客勒银子，把约瑟卖给以实玛利人。他们就把约瑟带到埃及去了。', ref: '创世记 37:28', hold: 8 },
  ];
  const V4 = [
    { text: '他们宰了一只公山羊，把约瑟的那件彩衣染了血，打发人送到他们的父亲那里……<br>他认得，就说：「这是我儿子的外衣。有恶兽把他吃了，约瑟被撕碎了！撕碎了！」', ref: '创世记 37:31–33', hold: 8.5 },
    { text: '雅各便撕裂衣服，腰间围上麻布，为他儿子悲哀了多日……<br>说：「我必悲哀着下阴间，到我儿子那里。」约瑟的父亲就为他哀哭。', ref: '创世记 37:34–35', hold: 8 },
    { text: '那时，犹大离开他弟兄下去……<br>犹大承认说：「她比我更有义……」', ref: '创世记 38:1、26', hold: 6 },
    { text: '约瑟被带下埃及去。有一个埃及人，是法老的内臣护卫长波提乏，<br>从那些带下他来的以实玛利人手下买了他去。', ref: '创世记 39:1', hold: 7.5 },
  ];
  const V5 = [
  ...
  const V6 = [
    { text: '这事以后，约瑟主人的妻以目送情给约瑟……<br>「……我怎能作这大恶，得罪神呢？」', ref: '创世记 39:7–9', hold: 7 },
    { text: '……约瑟把衣裳丢在妇人手里，跑到外边去了。', ref: '创世记 39:12', hold: 5 },
    { text: '……把约瑟下在监里，就是王的囚犯被囚的地方。于是约瑟在那里坐监。', ref: '创世记 39:20', hold: 6.5 },
    { text: '但耶和华与约瑟同在，向他施恩，使他在司狱的眼前蒙恩。', ref: '创世记 39:21', hold: 6.5 },
  ];
  const V7 = [
    { text: '他们对他说：「我们各人做了一梦，没有人能解。」<br>约瑟说：「解梦不是出于神吗？请你们将梦告诉我。」', ref: '创世记 40:8', hold: 7.5 },
    { text: '酒政便将他的梦告诉约瑟说：「我梦见在我面前有一棵葡萄树，<br>树上有三根枝子，好像发了芽，开了花，上头的葡萄都成熟了……」', ref: '创世记 40:9–10', hold: 8 },
    { text: '膳长……就对约瑟说：「我在梦中见我头上顶着三筐白饼……<br>有飞鸟来吃我头上筐子里的食物。」', ref: '创世记 40:16–17', hold: 7 },
    { text: '到了第三天，是法老的生日……<br>使酒政官复原职，他仍旧递杯在法老手中；但把膳长挂起来，正如约瑟向他们所解的话。', ref: '创世记 40:20–22', hold: 8.5 },
  ];
  const V8 = [
    { text: '酒政却不记念约瑟，竟忘了他。', ref: '创世记 40:23', hold: 4.5 },
    { text: '过了两年，法老做梦，梦见自己站在河边，<br>有七只母牛从河里上来，又美好又肥壮，在芦荻中吃草。', ref: '创世记 41:1–2', hold: 8 },
    { text: '随后又有七只母牛从河里上来，又丑陋又干瘦……<br>这又丑陋又干瘦的七只母牛吃尽了那又美好又肥壮的七只母牛。法老就醒了。', ref: '创世记 41:3–4', hold: 8.5 },
    { text: '他又睡着，第二回做梦，梦见一棵麦子长了七个穗子，又肥大又佳美，<br>随后又长了七个穗子，又细弱又被东风吹焦了。这细弱的穗子吞了那七个又肥大又饱满的穗子。', ref: '创世记 41:5–7', hold: 9.5 },
  ];
  const V9 = [
    { text: '到了早晨，法老心里不安，就差人召了埃及所有的术士和博士来……<br>却没有人能给法老圆解。', ref: '创世记 41:8', hold: 7 },
    { text: '那时酒政对法老说：「我今日想起我的罪来……」', ref: '创世记 41:9', hold: 4.8 },
    { text: '法老遂即差人去召约瑟，他们便急忙带他出监，<br>他就剃头，刮脸，换衣裳，进到法老面前。', ref: '创世记 41:14', hold: 7.5 },
    { text: '法老对约瑟说：「我做了一梦，没有人能解；我听见人说，你听了梦就能解。」<br>约瑟回答法老说：「这不在乎我，神必将平安的话回答法老。」', ref: '创世记 41:15–16', hold: 8.5 },
  ];
  const V10 = [
    ...
    { text: '法老对臣仆说：「像这样的人，有神的灵在他里头，我们岂能找得着呢？」', ref: '创世记 41:38', hold: 6 },
    { text: '法老又对约瑟说：「我派你治理埃及全地。」……<br>又叫约瑟坐他的副车，喝道的在前呼叫说：「跪下。」这样，法老派他治理埃及全地。', ref: '创世记 41:41–43', hold: 8.5 },
  ];
  const V11 = [
    { text: '七个丰年之内，地的出产极丰极盛，约瑟聚敛埃及地七个丰年一切的粮食，<br>把粮食积存在各城里……', ref: '创世记 41:47–48', hold: 7.5 },
    { text: '约瑟积蓄五谷甚多，如同海边的沙，无法计算，因为谷不可胜数。', ref: '创世记 41:49', hold: 6 },
    { text: '荒年未到以前，安城的祭司波提非拉的女儿亚西纳给约瑟生了两个儿子。<br>约瑟给长子起名叫玛拿西，因为他说：「神使我忘了一切的困苦和我父的全家。」', ref: '创世记 41:50–51', hold: 9 },
    { text: '他给次子起名叫以法莲，因为他说：「神使我在受苦的地方昌盛。」', ref: '创世记 41:52', hold: 6 },
  ];
  const V12 = [
    { text: '埃及地的七个丰年一完，七个荒年就来了。<br>正如约瑟所说的，各地都有饥荒；惟独埃及全地有粮食。', ref: '创世记 41:53–54', hold: 7.5 },
    { text: '当时饥荒遍满天下，约瑟开了各处的仓，粜粮给埃及人……<br>各地的人都往埃及去，到约瑟那里籴粮，因为天下的饥荒甚大。', ref: '创世记 41:56–57', hold: 8.5 },
    { text: '雅各见埃及有粮，就对儿子们说：「你们为什么彼此观望呢？<br>我听见埃及有粮，你们可以下去，从那里为我们籴些来，使我们可以存活，不至于死。」', ref: '创世记 42:1–2', hold: 8.5 },
    { text: '于是，约瑟的十个哥哥都下埃及籴粮去了。<br>但约瑟的兄弟便雅悯，雅各没有打发他和哥哥们同去……', ref: '创世记 42:3–4', hold: 6.5 },
  ];
  const V13 = [
    { text: '当时治理埃及地的是约瑟；粜粮给那地众民的就是他。<br>约瑟的哥哥们来了，脸伏于地，向他下拜。', ref: '创世记 42:6', hold: 7.5 },
    { text: '约瑟认得他哥哥们，他们却不认得他。<br>约瑟想起从前所做的那两个梦，就对他们说：「你们是奸细，来窥探这地的虚实。」', ref: '创世记 42:8–9', hold: 8 },
    { text: '约瑟转身退去，哭了一场……就从他们中间挑出西缅来，在他们眼前把他捆绑。<br>约瑟吩咐人……把各人的银子归还在各人的口袋里……', ref: '创世记 42:24–25', hold: 8 },
    { text: '……就对弟兄们说：「我的银子归还了，看哪，仍在我口袋里！」<br>……彼此说：「这是神向我们做什么呢？」', ref: '创世记 42:28', hold: 6.5 },
  ];
  const V14 = [
    { text: '他们的父亲雅各对他们说：「你们使我丧失我的儿子：约瑟没有了，西缅也没有了，<br>你们又要将便雅悯带去；这些事都归到我身上了。」', ref: '创世记 42:36', hold: 8.5 },
    { text: '那地的饥荒甚大。', ref: '创世记 43:1', hold: 4.2 },
    { text: '「但愿全能的神使你们在那人面前蒙怜悯，释放你们的那弟兄和便雅悯回来。<br>我若丧了儿子，就丧了吧！」', ref: '创世记 43:14', hold: 8.5 },
    { text: '于是，他们拿着那礼物，又手里加倍地带银子，并且带着便雅悯，<br>起身下到埃及，站在约瑟面前。', ref: '创世记 43:15', hold: 7.5 },
  ];
  const V15 = [
    { text: '约瑟举目看见他同母的兄弟便雅悯，就说：「你们向我所说那顶小的兄弟就是这位吗？」<br>又说：「小儿啊，愿神赐恩给你！」', ref: '创世记 43:29', hold: 8.5 },
    { text: '约瑟爱弟之情发动，就急忙寻找可哭之地，进入自己的屋里，哭了一场。', ref: '创世记 43:30', hold: 6.5 },
    { text: '约瑟把他面前的食物分出来，送给他们；但便雅悯所得的比别人多五倍。<br>他们就饮酒，和约瑟一同宴乐。', ref: '创世记 43:34', hold: 7 },
  ];
  const V16 = [
    { text: '家宰就搜查，从年长的起到年幼的为止，那杯竟在便雅悯的口袋里搜出来。', ref: '创世记 44:12', hold: 6.5 },
    { text: '犹大……说：「……现在求你容仆人住下，替这童子作我主的奴仆，<br>叫童子和他哥哥们一同上去。」', ref: '创世记 44:18、33', hold: 7.5 },
    { text: '约瑟在左右站着的人面前情不自禁……他就放声大哭……<br>约瑟对他弟兄们说：「我是约瑟。我的父亲还在吗？」', ref: '创世记 45:1–3', hold: 8 },
    { text: '他说：「我是你们的兄弟约瑟，就是你们所卖到埃及的。<br>现在，不要因为把我卖到这里自忧自恨。这是神差我在你们以先来，为要保全生命。」', ref: '创世记 45:4–5', hold: 9 },
  ];
  const V17 = [
    { text: '「……对他说：『你儿子约瑟这样说：神使我作全埃及的主，<br>请你下到我这里来……』」', ref: '创世记 45:9', hold: 7 },
    { text: '于是约瑟伏在他兄弟便雅悯的颈项上哭，便雅悯也在他的颈项上哭。<br>他又与众弟兄亲嘴，抱着他们哭，随后他弟兄们就和他说话。', ref: '创世记 45:14–15', hold: 8 },
    { text: '告诉他说：「约瑟还在，并且作埃及全地的宰相。」雅各心里冰凉，因为不信他们……<br>他们父亲雅各又看见约瑟打发来接他的车辆，心就苏醒了。', ref: '创世记 45:26–27', hold: 8.5 },
    { text: '以色列说：「罢了！罢了！我的儿子约瑟还在，趁我未死以先，我要去见他一面。」', ref: '创世记 45:28', hold: 6.5 },
  ];
  const V18 = [
    { text: '以色列带着一切所有的，起身来到别是巴，就献祭给他父亲以撒的神。', ref: '创世记 46:1', hold: 6.5 },
    { text: '夜间，神在异象中对以色列说：「雅各！雅各！」他说：「我在这里。」', ref: '创世记 46:2', hold: 6.5 },
    { text: '神说：「我是神，就是你父亲的神。你下埃及去不要害怕，<br>因为我必使你在那里成为大族。」', ref: '创世记 46:3', hold: 7.5 },
  ];
  const V19 = [
    { text: '「我要和你同下埃及去，也必定带你上来；约瑟必给你送终。」', ref: '创世记 46:4', hold: 6.5 },
    { text: '雅各就从别是巴起行。以色列的儿子们使他们的父亲雅各和他们的妻子、儿女都坐在法老为雅各送来的车上……<br>雅各和他的一切子孙都一同来了。', ref: '创世记 46:5–6', hold: 8.5 },
    { text: '……雅各家来到埃及的共有七十人。', ref: '创世记 46:27', hold: 4.5 },
    { text: '约瑟套车往歌珊去，迎接他父亲以色列，及至见了面，<br>就伏在父亲的颈项上，哭了许久。', ref: '创世记 46:29', hold: 7.5 },
  ];
  const V20 = [
  ...
  const V21 = [
    { text: '以色列伸出右手来，按在以法莲的头上（以法莲乃是次子），<br>又剪搭过左手来，按在玛拿西的头上（玛拿西原是长子）。', ref: '创世记 48:14', hold: 8 },
    { text: '他就给约瑟祝福说：「愿我祖亚伯拉罕和我父以撒所事奉的神，就是一生牧养我直到今日的神，<br>救赎我脱离一切患难的那使者，赐福与这两个童子……」', ref: '创世记 48:15–16', hold: 9.5 },
    { text: '以色列又对约瑟说：「我要死了，但神必与你们同在，领你们回到你们列祖之地。」', ref: '创世记 48:21', hold: 7 },
  ];
  const V22 = [
    { text: '雅各叫了他的儿子们来，说：「你们都来聚集，我好把你们日后必遇的事告诉你们。」', ref: '创世记 49:1', hold: 6.5 },
    { text: '「圭必不离犹大，杖必不离他两脚之间……万民都必归顺。」', ref: '创世记 49:10', hold: 6.5 },
    { text: '「约瑟是多结果子的树枝，是泉旁多结果的枝子；他的枝条探出墙外。」', ref: '创世记 49:22', hold: 6.5 },
    { text: '这一切是以色列的十二支派；这也是他们的父亲对他们所说的话，为他们所祝的福，<br>都是按着各人的福分为他们祝福。', ref: '创世记 49:28', hold: 9 },
  ];
  const V23 = [
    { text: '雅各嘱咐众子已毕，就把脚收在床上，气绝而死，归他列祖那里去了。', ref: '创世记 49:33', hold: 7 },
    { text: '约瑟伏在他父亲的面上哀哭，与他亲嘴。', ref: '创世记 50:1', hold: 5.5 },
  ];
  const V24 = [
    { text: '薰尸的常例是四十天；那四十天满了，埃及人为他哀哭了七十天。', ref: '创世记 50:3', hold: 6 },
    { text: '雅各的儿子们……把他搬到迦南地，葬在幔利前、麦比拉田间的洞里……', ref: '创世记 50:12–13', hold: 7 },
    { text: '约瑟葬了他父亲以后，就和众弟兄……都回埃及去了。', ref: '创世记 50:14', hold: 5 },
  ];
  const V25 = [
    { text: '约瑟的哥哥们见父亲死了，就说：<br>「或者约瑟怀恨我们，照着我们从前待他一切的恶足足地报复我们。」', ref: '创世记 50:15', hold: 7 },
    { text: '……他们对约瑟说这话，约瑟就哭了。<br>他的哥哥们又来俯伏在他面前，说：「我们是你的仆人。」', ref: '创世记 50:17–18', hold: 7.5 },
    { text: '约瑟对他们说：「不要害怕，我岂能代替神呢？<br>从前你们的意思是要害我，但神的意思原是好的，要保全许多人的性命，成就今日的光景。', ref: '创世记 50:19–20', hold: 9.5 },
    { text: '现在你们不要害怕，我必养活你们和你们的妇人孩子。」<br>于是约瑟用亲爱的话安慰他们。', ref: '创世记 50:21', hold: 6.5 },
  ];
  const V26 = [
    { text: '约瑟和他父亲的眷属都住在埃及。约瑟活了一百一十岁。<br>约瑟得见以法莲第三代的子孙。玛拿西的孙子、玛吉的儿子也养在约瑟的膝上。', ref: '创世记 50:22–23', hold: 9 },
    { text: '约瑟对他弟兄们说：「我要死了，但神必定看顾你们，领你们从这地上去……」<br>约瑟叫以色列的子孙起誓说：「……你们要把我的骸骨从这里搬上去。」', ref: '创世记 50:24–25', hold: 8.5 },
    { text: '约瑟死了，正一百一十岁。人用香料将他薰了，<br>把他收殓在棺材里，停在埃及。', ref: '创世记 50:26', hold: 7.5 },
  ];
  ...
  const KIDS = [['gk1', 0.724, '以法莲的子孙', 'm'], ['gk2', 0.752, '以法莲的子孙', 'f'], ['gk3', 0.73, '玛吉的儿子', 'm'], ['gk4', 0.758, '玛吉的儿子', 'f']];
  const CARAVAN = ['tr1', 'tr2', 'cam1', 'cam2', 'cam3'];

  // 话语：约瑟的故事里神多半不说话——所言说的，是神自己的话（46 章），
  // 或经上明说是神所行、所定的事（「耶和华与他同在」「神差我在你们以先来」「神的意思原是好的」）
  const STAGES = [
    // ── 1 · 约瑟做了一梦（37:5–8）────────────────────────────
    {
      ...
      apply(c) {
        ...
        T(c, [
          ...
          [L[2] + 7.5, () => walk('joseph', 0.852, { speed: 0.022 })],
        ]);
      },
    },
    ...
    // ── 2 · 梦见太阳、月亮与十一个星（37:9–11）─────────────────
    {
      ...
      apply(c) {
        ...
        T(c, [
          ...
          [L[2] + 5.5, () => face('jacob', -1)],
        ]);
      },
    },

    // ── 3 · 坑与二十舍客勒银子（37:12–28）── 「为要保全生命」（45:5）──
    //    哥哥们的意思是要害他；神的意思，是差他先去（45:5–8；50:20）
    {
      kind: 'act', utter: '为要保全生命', cmd: 'cd 埃及  # 你们的意思是要害我，但神的意思原是好的', ref: '45:5', tint: [255, 228, 184],
      verse: V3,
      apply(c) {
        const L = starts(V3);
        T(c, [
          [0, b => {
            ...
            pose('jacob', 'stand'); face('jacob', -1); pose('jacob', 'point');
            avoid([0.44, 0.72], [0.86, 0.93]);
          }],
          [2, () => { pose('jacob', 'stand'); walk('joseph', 0.664, { speed: 0.028 }); }],
          [4, () => { faceAll(BROS, 1); poseAll(['levi', 'simeon'], 'point'); }],
          [6.5, () => { poseAll(['levi', 'simeon'], 'stand'); poseAll(['dan', 'gad'], 'point'); }],
          [L[1], () => { poseAll(['dan', 'gad'], 'stand'); walk('reuben', 0.612, { speed: 0.03, pose: 'raise' }); }],
          [L[1] + 1.5, () => face('reuben', -1)],
          [L[1] + 3, () => walk('joseph', 0.628, { speed: 0.02 })],
          [L[1] + 6, () => pose('reuben', 'stand')],
          [L[2], b => {
            hold('joseph', null);
          ...
          [L[2] + 1, b => {
            ...
            walk('tr1', 0.686, { speed: 0.036 }); walk('cam1', 0.706, { speed: 0.036 });
            sfx(b, 'camel');
          }],
          [L[2] + 2.5, () => walk('joseph', X.pit + 0.004, { speed: 0.02 })],
          [L[2] + 4.5, () => { rm('joseph'); prop('pit', null, { lit: 1 }); }],
          [L[2] + 5.5, () => {
            walk('reuben', 0.47, { speed: 0.022, pose: 'sit' }); walk('simeon', 0.556, { speed: 0.025 }); walk('levi', 0.568, { speed: 0.025 });
            bros(id => { if (!['reuben', 'simeon', 'levi', 'judah'].includes(id)) pose(id, 'sit'); });
            pose('judah', 'stand'); walk('judah', 0.664, { speed: 0.03 });
          }],
          [L[3], b => {
            ...
            face('judah', 1); pose('judah', 'point');
          }],
          [L[3] + 2.5, b => { pose('judah', 'stand'); fxAdd(b, { type: 'coins', dur: 3, a: 'tr1', b: 'judah' }); }],
          [L[3] + 3.5, () => walk('joseph', 0.694, { speed: 0.03 })],
          [L[3] + 5, () => walk('reuben', X.pit + 0.013, { speed: 0.03, pose: 'weep' })],
          [L[3] + 6, () => {
            walk('tr1', 0.42, { speed: 0.036 }); follow('joseph', 'tr1', 0.016);
            walk('cam1', 0.452, { speed: 0.036 });
    ...
    // ── 4 · 约瑟被带下埃及去（37:31–39:1）────────────────────
    {
      ...
      apply(c) {
        ...
        T(c, [
          ...
          [5, b => { face('jacob', -1); pose('jacob', 'weep'); sfx(b, 'weep'); }],
          [6, () => { CARAVAN.concat(['joseph']).forEach(id => rm(id)); }],
          [L[1], b => {
            ...
            W.goTo(0.95, 8, b.instant);
          }],
          [L[2], () => walk('judah', 1.03, { speed: 0.035 })],
          [L[2] + 2, b => {
            // 一夜之间：左边的地化作埃及
          ...
          [L[2] + 5, () => walk('judah', 0.846, { speed: 0.03 })],
          [L[3] - 3, b => W.goTo(0.3, 6, b.instant)],
          [L[3], b => {
            const f = fromOf(b);
            add('tr1', { label: '以实玛利人', x: 0.46, facing: 1, robe: ROBE.trader, accent: [206, 186, 150], glow: 0.08, from: f });
            add('joseph', { label: '约瑟', sex: 'm', age: 'adult', scale: 0.94, x: 0.445, facing: 1, robe: ROBE.slave, glow: 0.42, prop: null, from: f });
            animal('cam1', 'camel', 0.425, { facing: 1, from: f, label: '骆驼' });
            animal('cam2', 'camel', 0.395, { facing: 1, from: f, follow: 'cam1', dx: 0.034, label: '骆驼' });
            animal('cam3', 'camel', 0.365, { facing: 1, from: f, follow: 'cam2', dx: 0.034, label: '骆驼' });
            add('tr2', { label: '以实玛利人', x: 0.345, facing: 1, robe: ROBE.trader2, accent: [180, 150, 110], glow: 0.08, from: f });
            follow('tr2', 'cam3', 0.022);
            follow('joseph', 'tr1', 0.016);
            walk('tr1', 0.588, { speed: 0.03 }); walk('cam1', 0.556, { speed: 0.03 });
            add('potiphar', { label: '波提乏', sex: 'm', age: 'adult', x: X.potiphar + 0.02, facing: -1, robe: ROBE.potiphar, accent: [176, 72, 52], glow: 0.12, from: f });
            avoid([0.5, 0.66], [0.78, 0.93]);
          }],
          [L[3] + 5, b => {
            unfollow('joseph'); walk('joseph', X.potiphar - 0.004, { speed: 0.022 });
            fxAdd(b, { type: 'coins', dur: 3, a: 'potiphar', b: 'tr1' });
          }],
          [L[3] + 7, () => {
            ['cam2', 'cam3', 'tr2'].forEach(unfollow);
            walk('tr2', 0.4, { speed: 0.034 }); walk('cam3', 0.425, { speed: 0.034 }); walk('cam2', 0.455, { speed: 0.034 });
            walk('cam1', 0.485, { speed: 0.034 }); walk('tr1', 0.51, { speed: 0.034 });
            face('joseph', 1);
          }],
          [L[3] + 7.4, () => CARAVAN.forEach(id => rm(id))],
        ]);
      },
    },

    // ── 5 · 耶和华与他同在，他就百事顺利（39:2–6）─────────────
    {
    ...
    // ── 6 · 耶和华与约瑟同在，向他施恩（39:7–21）───────────────
    {
      ...
      apply(c) {
        ...
        T(c, [
          ...
          [L[2] + 2, () => { walk('joseph', X.prison + 0.012, { speed: 0.03, pose: 'sit' }); rm('pwife'); unprop('garment'); }],
          [L[2] + 4, () => prop('prison', null, { open: 1 })],
          [L[2] + 6.5, b => { prop('prison', null, { lit: 1, open: 0 }); pose('potiphar', 'stand'); sfx(b, 'gate'); }],
          [L[3], b => {
            // 但耶和华与约瑟同在：监里有光
            ...
            pose('joseph', 'stand'); walk('joseph', X.prison - 0.004, { speed: 0.015 });
            pose('potiphar', 'stand'); walk('potiphar', X.potiphar + 0.012, { speed: 0.02 });
          }],
          [L[3] + 3.5, () => pose('keeper', 'bow')],
          [L[3] + 6, () => { pose('keeper', 'stand'); face('joseph', -1); }],
        ]);
      },
    },
    ...
    // ── 7 · 解梦不是出于神吗？（40）──────────────────────────
    {
      kind: 'ask', utter: '解梦不是出于神吗？', cmd: 'interpret 梦 --by 神  # 三根枝子 · 三个筐子 · 三天', ref: '40:8', tint: [232, 226, 255],
      verse: V7,
      apply(c) {
        const L = starts(V7);
        T(c, [
          [0, b => {
            W.goTo(0.95, 3, b.instant);
            prop('prison', null, { lit: 1, open: 0 });
            ...
            walk('cupbearer', X.prison - 0.03, { speed: 0.04, pose: 'lie' }); walk('baker', X.prison - 0.019, { speed: 0.04, pose: 'lie' });
            avoid([0.52, 0.74]);
          }],
          [2, () => { pose('joseph', 'lie'); crowdPose('prisoners', 'lie'); }],
          [4.5, b => W.goTo(0.3, 3.5, b.instant)],
          [6.5, () => { poseAll(['cupbearer', 'baker'], 'sit'); pose('joseph', 'stand'); face('joseph', -1); crowdPose('prisoners', 'sit'); }],
          [L[1] - 1, b => { W.set('jsDream', 0.55, b.instant); fxAdd(b, { type: 'vine', dur: 13 }); sfx(b, 'harp'); }],
          [L[1] + 5, b => { mote(b, 'joseph', 'cupbearer', [255, 226, 160], 3); pose('joseph', 'raise'); }],
          [L[1] + 8, () => pose('joseph', 'stand')],
          [L[2] - 1, b => { fxAdd(b, { type: 'baskets', dur: 10 }); face('joseph', 1); }],
          [L[2] + 4.5, () => pose('joseph', 'raise')],
          [L[2] + 7, () => pose('joseph', 'stand')],
          [L[3] - 1, b => { W.set('jsDream', 0, b.instant); W.goTo(0.46, 4, b.instant); }],
          [L[3], () => {
            prop('palace', null, { lit: 1 });
            pose('cupbearer', 'stand'); walk('cupbearer', X.palace + 0.028, { speed: 0.035 });
            pose('baker', 'stand'); walk('baker', 0.52, { speed: 0.022 }); glow('baker', 0.02);
          }],
          [L[3] + 5, () => rm('baker')],
          [L[3] + 7, b => { W.goTo(0.74, 7, b.instant); prop('palace', null, { lit: 0 }); walk('joseph', X.prison - 0.012, { speed: 0.015, pose: 'sit' }); }],
        ]);
      },
    },

    // ── 8 · 有七只母牛从河里上来（40:23–41:7）────────────────
    {
      ...
      apply(c) {
        ...
        T(c, [
          ...
          [L[1] - 0.8, b => { W.set('jsDream', 1, b.instant); fxAdd(b, { type: 'cows', dur: 23 }); sfx(b, 'harp'); }],
          [L[2] + 7.5, () => pose('pharaoh', 'sit')],
          [L[3] - 1.2, () => pose('pharaoh', 'lie')],
          [L[3] - 0.8, b => fxAdd(b, { type: 'ears', dur: 13.2, sp: 1.44 })],
          [L[4], b => { W.set('jsDream', 0, b.instant); pose('pharaoh', 'sit'); }],
        ]);
      },
    },

    // ── 9 · 神必将平安的话回答法老（41:8–16）─────────────────
    {
      kind: 'act', utter: '神必将平安的话回答法老', cmd: 'delegate 解梦 --to 神  # 这不在乎我', ref: '41:16', tint: [255, 234, 186],
      verse: V9,
      apply(c) {
        const L = starts(V9);
        T(c, [
          ...
          [2.5, () => { pose('pharaoh', 'seat'); crowdFace('magi', 1); }],
          [5, () => crowdPose('magi', 'bow')],
          [L[1], () => walk('cupbearer', X.throne - 0.02, { speed: 0.02, pose: 'bow' })],
          [L[1] + 2.5, () => { face('cupbearer', 1); pose('cupbearer', 'point'); }],
          [L[2], () => { prop('prison', null, { open: 1 }); pose('joseph', 'stand'); walk('keeper', X.prison + 0.022, { speed: 0.02 }); }],
          [L[2] + 1.5, b => {
          ...
          [L[2] + 4, () => { uncrowd('magi'); prop('prison', null, { open: 0 }); }],
          [L[2] + 6.5, () => { face('joseph', -1); face('pharaoh', 1); }],
          [L[3], b => {
          ...
          [L[3] + 3, () => pose('joseph', 'gaze')],
          [L[3] + 6.5, () => pose('joseph', 'stand')],
        ]);
      },
    },

    // ── 10 · 神已将所要做的事指示法老了（41:25–45）──────────────
    {
      kind: 'act', utter: '神已将所要做的事指示法老了', cmd: 'plan 7×丰年 7×荒年 --by 神  # 派他治理埃及全地', ref: '41:25', tint: [255, 234, 186],
      verse: V10,
      apply(c) {
        const L = starts(V10);
        T(c, [
          [0, b => {
            pose('joseph', 'raise'); const f = fig('joseph'); if (f) beam(b, f.nx, { dur: 8, w: 56, white: true }); sfx(b, 'harp');
            avoid([0.5, 0.76]);
          }],
          [4, () => pose('joseph', 'point')],
          [L[1] - 1, b => fxAdd(b, { type: 'years', dur: 13 })],
          [L[1] + 5, () => pose('joseph', 'stand')],
          [L[2], () => pose('pharaoh', 'stand')],
          [L[2] + 1.5, () => { pose('pharaoh', 'point'); crowdPose('court', 'bow'); }],
          [L[3] - 2.5, b => {
          ...
          [L[3], b => {
            ...
            walk('joseph', X.throne + 0.047, { speed: 0.02 });
          }],
          [L[3] + 2, () => { onChariot('joseph', 'chariot'); prop('chariot', null, { tx: 0.718 }); pose('pharaoh', 'seat'); crowdPose('court', 'stand'); }],
          [L[3] + 3, b => { crowdPose('people', 'kneel'); sfx(b, 'crowd'); }],
        ]);
      },
    },

    // ── 11 · 七个丰年之内，地的出产极丰极盛（41:46–52）──────────
    {
      ...
      apply(c) {
        ...
        beats.push([3.5 + 7 * 2.7, () => fields({ gold: 0.55 })]);
        beats.push([L[2] - 3, b => {
        ...
        beats.push([L[2] + 1, b => { carry('asenath', 'baby'); nameOver(b, 'asenath', '玛拿西', [255, 226, 176], srcAround('asenath', 50), { size: 0.036 }); }]);
        beats.push([L[3], b => {
          carry('asenath', null);
          add('manasseh', { label: '玛拿西', sex: 'm', age: 'child', x: X.jhouse + 0.026, facing: -1, robe: [226, 214, 190], glow: 0.25, from: fromOf(b) });
        }]);
        beats.push([L[3] + 1, b => { carry('asenath', 'baby'); nameOver(b, 'asenath', '以法莲', [214, 236, 176], srcAround('asenath', 50), { size: 0.036 }); }]);
        T(c, beats);
      },
    },

    // ── 12 · 七个荒年就来了（41:53–42:4）─────────────────────
    {
      ...
      apply(c) {
        ...
        T(c, [
          ...
          [L[2] + 3.5, () => { face('jacob', -1); pose('jacob', 'point'); faceAll(BROS, 1); }],
          [L[3], b => {
            pose('jacob', 'stand');
            const f = fromOf(b);
    ...
    // ── 13 · 赐给你们财宝在你们的口袋里（42:6–28；43:23）──────────
    //    众兄长脸伏于地（梦的回影）；西缅被捆；银子在口袋里——「这是神向我们做什么呢？」
    {
      kind: 'act', utter: '赐给你们财宝在你们的口袋里', cmd: 'git stash 银子 --into 口袋  # 这是神向我们做什么呢？', ref: '43:23', tint: [238, 238, 250],
      verse: V13,
      apply(c) {
        const L = starts(V13);
        T(c, [
          [0, () => { crowdWalk('hungry', 0.46, 0.58, { speed: 0.03 }); spread(BROS, 0.664, 0.742, { speed: 0.03, seed: 19 }); avoid([0.58, 0.76], [0.8, 0.92]); }],
          [2, () => uncrowd('hungry')],
          [3.5, b => {
          ...
          [L[2], () => { onDais(false); walk('joseph', 0.616, { speed: 0.03, pose: 'weep' }); }],
          [L[2] + 3.5, () => walk('joseph', X.seat, { speed: 0.03 })],
          [L[2] + 5, () => {
            onDais(true); face('joseph', 1); faceAll(BROS, -1); poseAll(BROS, 'stand');
            walk('simeon', 0.598, { speed: 0.025, pose: 'kneel' }); glow('simeon', 0.04);
          }],
          [L[2] + 6.5, b => {
            ['don1', 'don2', 'don3'].forEach(id => animal(id, 'donkey', null, { pack: true }));
            glint(b, 'don1', [236, 242, 255]);
          }],
          [L[2] + 7.5, () => { spread(noSimeon(), 0.82, 0.87, { speed: 0.03, seed: 23 }); face('simeon', 1); }],
          [L[3] + 0.5, b => {
            glint(b, 'don2', [236, 242, 255]); glint(b, 'don3', [236, 242, 255]);
            noSimeon().forEach((id, i) => face(id, i % 2 ? 1 : -1));
          }],
        ]);
      },
    },

    // ── 14 · 但愿全能的神使你们在那人面前蒙怜悯（42:36–43:15）──────
    {
      kind: 'bless', utter: '但愿全能的神使你们在那人面前蒙怜悯', cmd: 'retry 下埃及 --with 便雅悯  # 我若丧了儿子，就丧了吧', ref: '43:14', tint: [255, 236, 196],
      verse: V14,
      apply(c) {
        const L = starts(V14);
        T(c, [
          [0, () => { pose('jacob', 'sit', { weep: true }); walk('benjamin', X.jacob + 0.012, { speed: 0.02 }); avoid([0.58, 0.72], [0.8, 0.93]); }],
          [L[1], b => fxAdd(b, { type: 'dust', dur: 7 })],
          [L[1] + 2.5, () => { pose('jacob', 'stand'); face('jacob', -1); }],
          [L[2], b => { pose('jacob', 'raise'); const f = fig('jacob'); if (f) beam(b, f.nx, { dur: 7, w: 54 }); sfx(b, 'harp'); }],
          ...
          [L[3], b => {
            ...
            avoid([0.6, 0.8], [0.86, 0.93]);
          }],
          [L[3] + 4.5, () => { glow('simeon', 0.16); pose('simeon', 'stand'); walk('simeon', 0.728, { speed: 0.03 }); }],
        ]);
      },
    },

    // ── 15 · 小儿啊，愿神赐恩给你！（43:29–34）───────────────────
    {
      kind: 'bless', utter: '小儿啊，愿神赐恩给你！', cmd: 'serve 便雅悯 --portion ×5  # 和约瑟一同宴乐', ref: '43:29', tint: [255, 232, 188],
      verse: V15,
      apply(c) {
        const L = starts(V15);
        T(c, [
          [0, () => { walk('joseph', 0.7, { speed: 0.02 }); avoid([0.64, 0.82]); }],
          [2, b => { face('joseph', 1); pose('joseph', 'raise'); mote(b, 'joseph', 'benjamin', [255, 232, 180], 2.8); }],
          [6, () => pose('joseph', 'stand')],
          [L[1], () => walk('joseph', 0.676, { speed: 0.035, pose: 'weep' })],
          [L[1] + 5, () => walk('joseph', 0.699, { speed: 0.03 })],
          [L[2], b => {
          ...
          [L[2] + 2.5, b => { face('joseph', 1); mote(b, 'joseph', 'benjamin', [255, 226, 150], 2.5); }],
          [L[2] + 4, b => glint(b, 'benjamin', [255, 226, 150])],
        ]);
      },
    },

    // ── 16 · 神差我在你们以先来（44–45:5）──────────────────────
    //    银杯；犹大愿替便雅悯为奴；「我是约瑟」——放声大哭
    {
      ...
      apply(c) {
        ...
        T(c, [
          [0, b => {
            ...
            spread(eleven(), 0.79, 0.86, { speed: 0.035, seed: 37 });
            avoid([0.64, 0.9]);
          }],
          [1.5, () => run('steward', 0.78, {})],
          [3, () => { poseAll(eleven(), 'stand'); faceAll(eleven(), -1); }],
          [3.5, () => walk('steward', 0.845, { speed: 0.02 })],
          [5.5, b => {
          ...
          [L[1] - 1, b => { poseAll(eleven(), 'weep'); sfx(b, 'weep'); }],
          [L[1] + 1, () => { spread(eleven(), 0.716, 0.79, { speed: 0.034, seed: 41 }); walk('steward', X.jhouse + 0.008, { speed: 0.03 }); }],
          [L[1] + 4.5, () => { walk('joseph', 0.698, { speed: 0.02 }); poseAll(eleven(), 'fall'); }],
          [L[1] + 6, () => { face('joseph', 1); walk('judah', 0.708, { speed: 0.02, pose: 'kneel' }); glow('judah', 0.35); }],
          [L[1] + 7.5, () => { face('judah', -1); pose('judah', 'pray'); }],
          [L[2], () => { walk('steward', 0.6, { speed: 0.03 }); crowdWalk('people', 0.5, 0.6, { speed: 0.03 }); pose('judah', 'kneel'); }],
          [L[2] + 1, b => { pose('joseph', 'weep'); sfx(b, 'weep'); }],
          [L[2] + 3, () => { rm('steward'); uncrowd('people'); }],
          [L[2] + 4.5, b => {
            pose('joseph', 'stand');
            nameOver(b, 'joseph', '约瑟', [255, 230, 180], () => [rand(0.5, 1) * W.w, rand(0.05, 0.95) * W.h], { size: 0.06, hold: 3.2, lift: 1.2 });
            if (!b.instant) { const p = figPt('joseph', 0.6); if (p) fx().ring(p[0], p[1], [255, 232, 190], M() * 0.45, 2.6, 2.4); W.flash = 0.2; }
            poseAll(eleven(), 'kneel');
          }],
          [L[2] + 7, () => eleven().forEach((id, i) => { const f = fig(id); if (f && i % 2) walk(id, f.nx + 0.008, { speed: 0.03, pose: 'kneel' }); })],
          [L[3], () => pose('joseph', 'raise')],
          [L[3] + 2, () => { poseAll(eleven(), 'stand'); spread(eleven(), 0.716, 0.77, { speed: 0.02, seed: 43 }); pose('joseph', 'stand'); }],
          [L[3] + 4.5, b => { const f = fig('joseph'); if (f) beam(b, f.nx, { dur: 9, w: 70 }); sfx(b, 'harp'); }],
        ]);
      },
    },

    // ── 17 · 神使我作全埃及的主（45:9–28）──────────────────────
    {
      kind: 'act', utter: '神使我作全埃及的主', cmd: 'git push 车辆 --to 迦南  # 请你下到我这里来', ref: '45:9', tint: [255, 230, 186],
      verse: V17,
      apply(c) {
        const L = starts(V17);
        T(c, [
          [0, () => { face('joseph', 1); pose('joseph', 'point'); avoid([0.66, 0.93]); }],
          [4, () => pose('joseph', 'stand')],
          [L[1], () => embrace('joseph', 'benjamin', { weep: true })],
          [L[1] + 4, () => eleven().forEach(id => { if (id !== 'benjamin') pose(id, 'weep'); })],
          [L[1] + 7, () => { poseAll(eleven(), 'stand'); pose('joseph', 'stand'); }],
          [L[2] - 2.5, b => {
            // 45:21 约瑟照着法老的吩咐给他们车辆
            const f = fromOf(b);
          ...
          [L[2] - 1.5, () => {
            spread(eleven(), 0.815, 0.87, { speed: 0.035, seed: 47 });
            walk('ox1', 0.965, { speed: 0.035 }); walk('ox2', 0.93, { speed: 0.035 });
            walk('joseph', X.jhouse - 0.01, { speed: 0.02 });
          }],
          [L[2] + 2, () => { faceAll(eleven(), 1); pose('jacob', 'sit'); face('jacob', -1); }],
          [L[2] + 6, b => { pose('jacob', 'stand'); glow('jacob', 0.6); if (!b.instant) { const p = figPt('jacob', 0.5); if (p) fx().ring(p[0], p[1], [255, 236, 200], M() * 0.3, 2.2, 2); } }],
          [L[3], () => pose('jacob', 'raise')],
          [L[3] + 4.5, () => pose('jacob', 'stand')],
        ]);
      },
    },
    ...
    // ── 18 · 雅各！雅各！（46:1–3）── 神的话 ───────────────────
    {
      kind: 'call', utter: '雅各！雅各！', cmd: 'ping 以色列 --at 别是巴  # 我在这里', ref: '46:2', tint: [255, 240, 206], hold: 2.6,
      verse: V18,
      apply(c) {
        const L = starts(V18);
        T(c, [
          ...
          [3.5, b => { prop('altar', null, { fire: 1 }); sfx(b, 'fire'); }],
          [L[1] - 0.5, b => {
            W.goTo(0.97, 4, b.instant);
            pose('jacob', 'pray');
            crowdPose('house', 'sit'); bros(id => pose(id, 'sit')); pose('benjamin', 'sit');
            beam(b, X.beersheba + 0.014, { dur: 15, w: 80, white: true, k: 1.1 }); W.set('jsDream', 0.4, b.instant); sfx(b, 'harp');
          }],
          [L[1] + 2, () => {
            // 夜里：牛调过头来，车朝着埃及
          ...
          [L[2], b => { if (!b.instant) { const p = figPt('jacob', 0.5); if (p) fx().ring(p[0], p[1], [255, 240, 210], M() * 0.5, 2.8, 2); } }],
        ]);
      },
    },

    // ── 19 · 我要和你同下埃及去（46:4–29）── 神的话 ──────────────
    {
      ...
      apply(c) {
        const L = starts(V19);
        T(c, [
          [0, b => { if (!b.instant) { const p = figPt('jacob', 0.5); if (p) fx().ring(p[0], p[1], [255, 240, 210], M() * 0.35, 2.4, 2); } avoid([0.7, 0.97]); }],
          [2.5, b => { W.set('jsDream', 0, b.instant); W.goTo(0.3, 5, b.instant); prop('altar', null, { fire: 0.15 }); }],
          [4, () => { pose('jacob', 'stand'); walk('jacob', 0.93, { speed: 0.04 }); }],
          [6.5, () => ride('jacob', 'wagon1')],
          [L[1], () => {
          ...
          [L[2], b => fxAdd(b, { type: 'seventy', dur: 7, x0: 0.72, x1: 0.81 })],
          [L[3] - 1.5, () => {
            prop('chariot2', 'chariot', { x: X.jhouse - 0.01, spd: 0.03, label: '车' });
            onChariot('joseph', 'chariot2');
            prop('chariot2', null, { tx: 0.724 });
          }],
          [L[3] + 0.5, () => { ride('jacob', null); walk('jacob', 0.752, { speed: 0.024 }); }],
          [L[3] + 2, () => { onChariot('joseph', null); place('joseph', 0.73); }],
          [L[3] + 3, () => embrace('joseph', 'jacob', { weep: true })],
          [L[3] + 7, () => { pose('joseph', 'stand'); pose('jacob', 'stand'); }],
        ]);
      },
    },

    // ── 20 · 我必使你在那里成为大族（47）── 神的话（46:3），在歌珊应验 ──
    {
      kind: 'promise', utter: '我必使你在那里成为大族', cmd: 'chown -R 以色列 ./歌珊  # 生育甚多', ref: '46:3', tint: [236, 246, 206],
      verse: V20,
      apply(c) {
        const L = starts(V20);
        T(c, [
          ...
          [L[2] + 2.5, b => {
            crowd('house2', { n: 11, x0: 0.775, x1: 0.84, layer: 2, label: '以色列人', from: fromOf(b) }); sfx(b, 'bleat');
            ['ox1', 'ox2', 'wagon1', 'wagon2', 'don1', 'don2', 'don3'].forEach(id => rm(id));
            if (!b.instant) { const p = [0.8 * W.w, fieldY(0.8, 0.2)]; fx().ring(p[0], p[1], [236, 246, 206], M() * 0.3, 2.4, 2); }
          }],
          [L[3], b => { passDay(b, 6); prop('bed', 'bed', { x: X.bed, label: '雅各的床' }); }],
          [L[3] + 2, () => walk('jacob', X.bed, { speed: 0.02 })],
          [L[3] + 3.5, () => { onBed('jacob', true); pose('jacob', 'sit'); face('jacob', -1); }],
        ]);
      },
    },
      ...
      apply(c) {
        ...
        T(c, [
          [0, () => {
            ...
            walk('joseph', X.bed - 0.036, { speed: 0.03 });
            walk('manasseh', X.bed - 0.022, { speed: 0.03 }); walk('ephraim', X.bed - 0.013, { speed: 0.03 });
            avoid([0.68, 0.84]);
          }],
          [2.5, b => {
          ...
          [L[1], b => { beam(b, X.bed, { dur: 9, w: 60 }); sfx(b, 'harp'); }],
          [L[2], () => pose('jacob', 'sit')],
          [L[2] + 1, () => { face('jacob', 1); pose('jacob', 'point'); }],
          [L[2] + 5.5, () => { pose('jacob', 'sit'); face('jacob', -1); }],
        ]);
      },
    },

    // ── 22 · 我必使你生养众多，成为多民（49:1–28）── 神的话（48:4）：十二支派 ──
    {
      kind: 'promise', utter: '我必使你生养众多，成为多民', cmd: 'fork 以色列 --into 十二支派  # 按着各人的福分', ref: '48:4', tint: [236, 232, 255],
      verse: V22,
      apply(c) {
        const L = starts(V22);
        ...
        T(c, [
          ...
          [5, () => SONS12.forEach(id => face(id, POS[id] < X.bed ? 1 : -1))],
    ...
    // ── 23 · 约瑟必给你送终（49:33–50:1）── 神的话（46:4），此刻应验 ──
    {
      kind: 'promise', utter: '约瑟必给你送终', cmd: 'return 雅各 → 列祖  # 约瑟伏在他父亲的面上', ref: '46:4', tint: [228, 230, 250],
      verse: V23,
      apply(c) {
        const L = starts(V23);
        T(c, [
          [0.5, () => { pose('jacob', 'lie'); glow('jacob', 0.05); avoid([0.66, 0.88]); }],
          [2.5, b => { soul(b, 'jacob', 6); W.goTo(0.96, 7, b.instant); }],
          [L[1] - 1, b => {
    ...
    // ── 24 · 也必定带你上来（50:2–14）── 神的话（46:4）：葬在麦比拉洞 ──
    {
      kind: 'promise', utter: '也必定带你上来', cmd: 'mv 雅各 迦南/麦比拉洞  # 与列祖同葬', ref: '46:4', tint: [236, 232, 214],
      verse: V24,
      apply(c) {
        const L = starts(V24);
        T(c, [
          ...
          [L[1] + 9, b => { unprop('bier'); prop('cave', null, { seal: 1 }); sfx(b, 'seal'); }],
          [L[2] + 1.5, () => {
    ...
    // ── 25 · 神的意思原是好的（50:15–21）──────────────────────
    {
      ...
      apply(c) {
        const L = starts(V25);
        T(c, [
          [0, b => { W.goTo(0.56, 5, b.instant); walk('joseph', 0.703, { speed: 0.025 }); spread(eleven(), 0.724, 0.79, { speed: 0.025, seed: 61 }); avoid([0.66, 0.84]); }],
          [5, () => { faceAll(eleven(), -1); face('joseph', 1); }],
          ...
          [L[3] + 6, () => { pose('joseph', 'stand'); pose('reuben', 'stand'); }],
        ]);
      },
    },

    // ── 26 · 神必定看顾你们（50:22–26）── 全书的最后一句 ──────────
    //    末一幅：黄昏，约瑟的棺材停在埃及，一道柔光在上；众人退开，应许之路在右边微微亮着
    {
      ...
      apply(c) {
        ...
        T(c, [
          ...
          [2.5, b => {
            walk('joseph', X.coffin, { speed: 0.02, pose: 'sit' });
            KIDS.forEach(([id, x, label, sex], i) => add(id, { label, sex, age: 'child', x, facing: x < X.coffin ? 1 : -1, robe: [206 - i * 8, 178 + i * 4, 140], glow: 0.2, v: 0.12 + 0.08 * i, from: fromOf(b) }));
          }],
          [5.5, () => { carry('joseph', 'baby'); face('joseph', 1); }],
          [L[1], b => {
            carry('joseph', null); pose('joseph', 'stand'); face('joseph', 1);
            W.goTo(0.735, 10, b.instant);             // 日落：50:26 在黄昏里读出
            // 众人退开，给末一幅留出地方：左边是约瑟的家，右边是众弟兄与以色列人
            spread(eleven(), 0.795, 0.87, { speed: 0.03, seed: 67 });
            KIDS.forEach(([id], i) => walk(id, 0.632 + i * 0.008, { speed: 0.03 }));
            walk('manasseh', 0.682, { speed: 0.025 }); walk('ephraim', 0.67, { speed: 0.025 }); walk('asenath', 0.694, { speed: 0.025 });
            crowdWalk('house', 0.878, 0.93, { speed: 0.025 }); crowdWalk('house2', 0.886, 0.95, { speed: 0.025 });
            crowdWalk('flockC', 0.9, 0.97, { speed: 0.025 });
          }],
          [L[1] + 1.2, b => { pose('joseph', 'point'); W.set('jsPromise', 1, b.instant); sfx(b, 'harp'); }],
          [L[1] + 4, () => { poseAll(eleven(), 'raise'); KIDS.forEach(k => face(k[0], 1)); }],
          [L[1] + 7.5, () => { poseAll(eleven(), 'stand'); pose('joseph', 'stand'); }],
          // 黄昏缓缓延长到全书终的字出现之后（之后时辰才自行流转入夜）
          [L[2], b => { pose('joseph', 'lie'); W.set('jsWith', 0, b.instant); W.goTo(0.775, 26, b.instant); }],
          [L[2] + 1.2, b => { const p = figPt('joseph', 0.3); if (p) fxAdd(b, { type: 'soul', dur: 4.5, x0: p[0] / W.w, y0: p[1] / W.h, up: 0.06 }); }],
          [L[2] + 2.6, () => {
            rm('joseph');
            prop('coffin', 'coffin', { x: X.coffin, size: 2.1, lit: 1, label: '约瑟的棺材' });
  ...
  const TRIBES = { text: '这一切是以色列的十二支派；这也是他们的父亲对他们所说的话，为他们所祝的福，<br>都是按着各人的福分为他们祝福。', ref: '创世记 49:28' };
  GS.book.act({
    // 幕（全书终的字）要等末一句话的经文与故事都尽了才出现；outro 让停在埃及的棺材在黄昏里多留一会儿
    id: ACT, title: '约瑟', sub: '创世记 37:1 — 50:26', tint: [255, 222, 164], outro: 32,
    intro: INTRO,
    // 全书终后，按住本卷的人与物，显出与它相关的经文
    behold: {
      '约瑟的棺材': { text: '约瑟死了，正一百一十岁。人用香料将他薰了，<br>把他收殓在棺材里，停在埃及。', ref: '创世记 50:26' },
      '约瑟': { text: '「约瑟是多结果子的树枝，是泉旁多结果的枝子；他的枝条探出墙外。」', ref: '创世记 49:22' },
      '雅各': { text: '雅各住在埃及地十七年，雅各平生的年日是一百四十七岁。', ref: '创世记 47:28' },
      '便雅悯': { text: '约瑟举目看见他同母的兄弟便雅悯……又说：「小儿啊，愿神赐恩给你！」', ref: '创世记 43:29' },
      '吕便': { text: '吕便听见了，要救他脱离他们的手，说：「我们不可害他的性命。」', ref: '创世记 37:21' },
      '西缅': { text: '……就从他们中间挑出西缅来，在他们眼前把他捆绑。', ref: '创世记 42:24' },
      '犹大': { text: '「圭必不离犹大，杖必不离他两脚之间……万民都必归顺。」', ref: '创世记 49:10' },
      '亚设': { text: '「亚设之地必出肥美的粮食，且出君王的美味。」', ref: '创世记 49:20' },
      '利未': TRIBES, '西布伦': TRIBES, '以萨迦': TRIBES, '但': TRIBES, '迦得': TRIBES, '拿弗他利': TRIBES,
      '以色列的十二支派': TRIBES,
      '玛拿西': { text: '约瑟给长子起名叫玛拿西，因为他说：「神使我忘了一切的困苦和我父的全家。」', ref: '创世记 41:51' },
      '以法莲': { text: '他给次子起名叫以法莲，因为他说：「神使我在受苦的地方昌盛。」', ref: '创世记 41:52' },
      '亚西纳': { text: '……又将安城的祭司波提非拉的女儿亚西纳给他为妻。', ref: '创世记 41:45' },
      '法老': { text: '法老对臣仆说：「像这样的人，有神的灵在他里头，我们岂能找得着呢？」', ref: '创世记 41:38' },
      '以法莲的子孙': { text: '约瑟得见以法莲第三代的子孙。', ref: '创世记 50:23' },
      '玛吉的儿子': { text: '玛拿西的孙子、玛吉的儿子也养在约瑟的膝上。', ref: '创世记 50:23' },
      '以色列人': { text: '以色列人住在埃及的歌珊地。他们在那里置了产业，并且生育甚多。', ref: '创世记 47:27' },
      '以色列人的帐棚': { text: '以色列人住在埃及的歌珊地。他们在那里置了产业，并且生育甚多。', ref: '创世记 47:27' },
      '雅各的家眷': { text: '……雅各家来到埃及的共有七十人。', ref: '创世记 46:27' },
      '仓城': { text: '约瑟积蓄五谷甚多，如同海边的沙，无法计算，因为谷不可胜数。', ref: '创世记 41:49' },
      '约瑟的座': { text: '当时治理埃及地的是约瑟；粜粮给那地众民的就是他。', ref: '创世记 42:6' },
      '尼罗河': { text: '有七只母牛从河里上来，又美好又肥壮，在芦荻中吃草。', ref: '创世记 41:2' },
      '麦比拉洞': { text: '雅各的儿子们……把他搬到迦南地，葬在幔利前、麦比拉田间的洞里……', ref: '创世记 50:12–13' },
      '雅各的帐棚': { text: '雅各住在迦南地，就是他父亲寄居的地。', ref: '创世记 37:1' },
    },
