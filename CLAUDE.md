...
- 唯一的入口是根目录的 `index.html`：单文件，样式、脚本、字体全部内嵌，由 `build.py` 生成，不要手改。
- 源码全部在 `src/`（`src/index.html` 引用 `src/css`、`src/js`、`src/fonts`）。
- 每次改了 `src/` 里的任何东西，都要运行 `python3 build.py` 重新生成根目录的 `index.html`，并一起提交。
